...
# Rule
## MUST
- The `./ai-worktree` directory must be added to the main repository's `.gitignore`. This prevents worktree service files from being tracked by git.
...
# Anti-patterns
>[!hint]
>Typical mistakes and the consequences they lead to
...
- **Not adding `./ai-worktree` to `.gitignore`**
  - Worktree directories will appear in git as untracked files, clutter the repository, and may be accidentally committed.
