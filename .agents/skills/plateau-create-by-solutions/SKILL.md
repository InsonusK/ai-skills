...
# Prerequisites
Read [solution-create](.agents/skills/solution-create/SKILL.md) first. It defines how a solution-skill is structured and what files it produces. A plateau is built by aggregating those produced files across all selected solutions.
...
4. Discover all projects and classes contributed by {solutions}
   ...
   - Normalize `{Module}` projects to generic module templates (see [Project name normalization](.agents/skills/plateau-create-by-solutions/SKILL.md#project-name-normalization))
5. Create `sln-{plateau-name}.skill.md` using [sln-{name}.skill.template.md](.agents/skills/plateau-create-by-solutions/templates/sln-{name}.skill.template.md)
   - Aggregate all `Repository.create.md` files from {solutions}
   - Keep repository-level content only
6. For each discovered project create `csproj-{normalized-name}.skill.md` using [csproj-{name}.skill.template.md](.agents/skills/plateau-create-by-solutions/templates/csproj-{name}.skill.template.md)
   - Merge `.create.md` and all `.extend.md` files for the same project
   - Keep project-level content only
7. For each discovered class create `class-{normalized-name}.skill.md` using [class-{name}.skill.template.md](.agents/skills/plateau-create-by-solutions/templates/class-{name}.skill.template.md)
   - Merge `.cs.create.md` and `.cs.extend.md` files for the same class
   - Keep class-level content only
8. Create `plateau-{plateau-name}.skill.md` using [plateau-{name}.skill.template.md](.agents/skills/plateau-create-by-solutions/templates/plateau-{name}.skill.template.md)
...
- solutions:
  - [solution-sln-structure](./files/solution-sln-structure.skill.md)
  - [solution-command-integration](./files/solution-command-integration.skill.md)
  - [solution-entity-concurrency-change](./files/solution-entity-concurrency-change.skill.md)
