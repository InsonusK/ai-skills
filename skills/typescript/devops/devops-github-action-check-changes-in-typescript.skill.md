      ...
      with:
        filters: |
          ...
          docs:
            ...
            - '*.md'
```
- Violation: filtering only `src/**` for `test`, missing `features/**` (Cucumber) or `*.spec.ts`/`*.test.ts` (Vitest) — the two suites [ts-solution-conformance-testing](skills/typescript/architecture/solutions/ts-solution-conformance-testing.skill/ts-solution-conformance-testing.skill.md) actually runs.
