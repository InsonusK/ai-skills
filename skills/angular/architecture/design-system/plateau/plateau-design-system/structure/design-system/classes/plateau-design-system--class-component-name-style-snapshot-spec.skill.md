...
> `projects/design-system/src/lib/{component}/spec/{component}.style-snapshot.spec.ts`. Navigates to the same preview page as `.visual.spec.ts`, reads a computed-style snapshot via the shared [[skills/angular/architecture/design-system/plateau/plateau-design-system/structure/design-system/classes/plateau-design-system--class-read-visual-style-properties.skill.md|read-visual-style-properties]] helper. Text baselines commit to `spec/snapshot/`.
