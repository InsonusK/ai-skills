...
# Involved solutions
Project created by `solution-repository-structure` as part of the monolith workspace. `.extend`ed, in the composed monolith plateau, by `app-routing` / `performance-tuned-routing` / `offline-first` / `offline-sync` / `logging-global` / `authentication` — see [`plateau-multiuser-monolith/registry/platform-shell-project`](skills/angular/architecture/monolith/plateau/plateau-multiuser-monolith/registry/platform-shell-project.md). This plateau adds:
