...
This is not tied to one concrete feature. Any solution that creates a new `libs/{feature}/feature` project (see solution #1's structure) follows this pattern for that feature's own state, substituting `{Feature}`/`{feature}` with the real feature name.
...
- All HTTP/data access invoked from the store MUST go through that feature's own `data-access` lib (see the future "API/HTTP-слой" solution), never directly via `HttpClient` in the store.

## SHOULD
- Genuinely global reads (e.g. current user) SHOULD come from `libs/shared/state` selectors, injected into the feature store, rather than being duplicated locally (see [[../../../Implementation/GlobalStore/shared-state.project.create/auth.store.ts.create.md]]).
