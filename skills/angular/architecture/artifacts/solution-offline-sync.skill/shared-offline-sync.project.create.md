...
- Durable, reactive storage for queued mutations, partitioned by feature per [[../adr/queue-partitioning-and-ordering.md]]
...
| replay-orchestrator.ts | Triggered by the `connectivity` slice's `isOnline` transitioning to `true` (from the "Offline-first" solution); replays each feature's partition independently and in parallel, FIFO within a partition, per [[../adr/queue-partitioning-and-ordering.md]]. |
...
- `touchedFields` MUST be stored per queued mutation at enqueue time — it is derived directly from the command's own payload, never a separately captured entity snapshot, per [[../adr/conflict-resolution-strategy.md]].
