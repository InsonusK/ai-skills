...
- The Client MUST catch every `HttpErrorResponse` its calls can produce and rethrow one of this feature's typed domain errors from `{feature}.errors.ts` — never let the raw error escape, per [[../../../adr/error-handling-strategy.md]].
- DTO ↔ domain model mapping MUST go through this feature's `{feature}.mapper.ts` hand-written functions, per [[../../../adr/dto-mapping-strategy.md]] — never inlined ad hoc inside the Client method.
