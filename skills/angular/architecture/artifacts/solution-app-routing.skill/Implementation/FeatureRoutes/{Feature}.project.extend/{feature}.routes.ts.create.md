...
- **Baking the feature's expected mount segment into its own route paths (e.g. `path: 'orders/:id'`)**
  - Consequence: breaks the moment the feature is mounted under a different segment or nested inside an embeddable module instead of directly under the shell
