...
REPOSITORY:
- [[./Implementation/Repository.extend.md|Repository]] - extend - add the convention that every routable feature exports its `Routes` via `index.ts`, relative to its own root only
PROJECT:
- [[./Implementation/PlatformHost/platform-shell.project.extend.md|apps/platform-shell]] - extend - root `app.routes.ts` mounting module/feature root segments only
- [[./Implementation/EmbeddableModule/routes.ts.extend.md|Embeddable module (generic pattern)]] - extend - mounts its own features' root segments, applied by any multi-feature embeddable module
- [[./Implementation/FeatureRoutes/{Feature}.project.extend/{feature}.routes.ts.create.md|{Feature}/feature (generic pattern)]] - create - feature's own root-relative routes, exported from `index.ts`, applied by any future feature-owning solution
...
1. The embeddable module exposes its own root-relative routes as part of its federation entry point (see [[./Implementation/EmbeddableModule/routes.ts.extend.md]]).
...
2. This is caught in review against the rule in [[./Implementation/Repository.extend.md#MUST NOT]] — the shell must not reference a path two or more levels below its own mount point.
...
- [[./Implementation/EmbeddableModule/routes.ts.extend.md#MUST|EmbeddableModule/routes.ts.extend]]
- [[./Implementation/FeatureRoutes/{Feature}.project.extend/{feature}.routes.ts.create.md#MUST|{feature}.routes.ts.create]]
...
- [[./Implementation/Repository.extend.md|See Repository.extend.md]] — a feature baking its own mount segment into its routes; the shell reaching into a feature's internal path structure.
- [[./Implementation/PlatformHost/platform-shell.project.extend.md|See platform-shell.project.extend.md]] — the shell defining a nested path instead of mounting a single root segment.
- [[./Implementation/EmbeddableModule/routes.ts.extend.md|See routes.ts.extend.md]] — a module hardcoding its own expected mount prefix.
- [[./Implementation/FeatureRoutes/{Feature}.project.extend/{feature}.routes.ts.create.md|See {feature}.routes.ts.create.md]] — a feature baking its own name into its own route paths.
