...
## MUST
- Every Nx project MUST run its unit/component tests via Vitest — no project may configure Karma or Jest as its test runner, per [[../adr/test-runner-choice.md]].
- End-to-end tests MUST be written with Playwright, in a dedicated `type:e2e` project, per [[../adr/e2e-framework-choice.md]].
- `HttpTestingController` MUST be used only inside a feature's own `{feature}.client.ts` unit tests — no other test may use it, per [[../adr/testing-layers-and-mocking.md]].
