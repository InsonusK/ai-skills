...
| --------------- | ----------- |
| /projects/design-system/src/lib/{component-name} | One directory per component, following the generic pattern in [[./ComponentLayer/{component-name}.component.ts.create.md]]. |

# Rules

## MUST
- Every component MUST use the `ds-` selector prefix — never expose or re-export an Angular Material selector directly.
- Every component's public API MUST use `input()`, `output()`, and `model()` — no `@Input()`/`@Output()` decorators, no `EventEmitter`, per [[../adr/component-api-authoring-style.md]].
- Every component's API MUST be designed around this application's actual usage axes — it MUST NOT mirror Angular Material's own input names or category model 1:1, per [[../adr/component-encapsulation-strategy.md]].
