...
SOLUTION:
- [[../solution-app-routing.skill/solution-app-routing.skill.md|App routing (база)]]
  - [[../solution-app-routing.skill/Implementation/PlatformHost/platform-shell.project.extend.md|apps/platform-shell (app.routes.ts)]] - top-level segments gain the `data.preload` flag at their mounting point
  - [[../solution-app-routing.skill/Implementation/FeatureRoutes/{Feature}.project.extend/{feature}.routes.ts.create.md|{Feature}/feature routes]] - feature's own sub-routes gain `loadComponent` splitting where justified
...
REPOSITORY:
- [[./Implementation/Repository.extend.md|Repository]] - extend - add enforced bundle budgets and the `data.preload` mounting-point convention
PROJECT:
- [[./Implementation/PlatformHost/platform-shell.project.extend.md|apps/platform-shell]] - extend - register a custom `SelectivePreloadingStrategy`, mark selected top-level segments `preload: true`
- [[./Implementation/FeatureRoutes/{Feature}.project.extend/{feature}.routes.ts.extend.md|{Feature}/feature routes (generic pattern)]] - extend - rule for splitting heavy/rare sub-routes via `loadComponent`, plus the feature's own chunk budget
...
2. The `type:app` project's initial-bundle budget (declared per [[./Implementation/Repository.extend.md#MUST]]) is exceeded.
...
- [[./Implementation/FeatureRoutes/{Feature}.project.extend/{feature}.routes.ts.extend.md#MUST|{feature}.routes.ts.extend]]

## SHOULD
- [[./Implementation/Repository.extend.md#SHOULD|Repository.extend]]
- [[./Implementation/FeatureRoutes/{Feature}.project.extend/{feature}.routes.ts.extend.md#SHOULD|{feature}.routes.ts.extend]]

## SHOULD NOT
- [[./Implementation/FeatureRoutes/{Feature}.project.extend/{feature}.routes.ts.extend.md#SHOULD NOT|{feature}.routes.ts.extend]]
...
- [[./Implementation/Repository.extend.md|See Repository.extend.md]] — silencing a bundle budget failure by raising the threshold; a feature setting its own `preload` flag.
- [[./Implementation/PlatformHost/platform-shell.project.extend.md|See platform-shell.project.extend.md]] — marking every top-level segment `preload: true`, degenerating into `PreloadAllModules`.
- [[./Implementation/FeatureRoutes/{Feature}.project.extend/{feature}.routes.ts.extend.md|See {feature}.routes.ts.extend.md]] — splitting every sub-route via `loadComponent` regardless of actual size/usage, or leaving a genuinely heavy sub-page unsplit in the main chunk.
