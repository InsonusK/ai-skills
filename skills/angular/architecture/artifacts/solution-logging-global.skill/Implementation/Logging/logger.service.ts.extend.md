...
## MUST
- `report()` entries MUST always reach `BackendLogSink` (per [[../../adr/backend-log-sink-strategy.md]]), regardless of `MIN_LOG_LEVEL` — the level filtering that suppresses `debug`/`info` in production does not apply to `report()`.
