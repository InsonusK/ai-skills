...
This applies to any embeddable app repository (per the "Встраиваемость платформы" solution's "встраиваемое приложение" plateau), extending the baseline structure that solution already establishes.
...
| /src/styles.scss | Imports the design system's `theme.scss`/`custom-tokens.scss`, used when this app is run standalone during local development — per [[../../adr/theme-application-scope.md]], this import is redundant (but harmless) once the app is mounted inside the platform in production. |
...
- **Declaring an unbounded or overly permissive `requiredVersion` (e.g. accepting any major version)**
...
- **Never updating `requiredVersion` after the initial setup**
