...
| /projects/demo | A plain Angular application, not published, importing `design-system` as a regular dependency and rendering example usage of every component — the project's answer to component preview/visual review, per [[../adr/component-preview-tooling.md]]. |
| /.changeset | Changesets configuration and pending changeset files, per [[../adr/release-versioning-strategy.md]]. |

# Rules

## MUST
- The repository MUST be a plain Angular CLI multi-project workspace, not an Nx workspace, per [[../adr/design-system-workspace-tooling.md]].
- The library project MUST be built with ng-packagr, per [[../adr/library-build-tooling.md]] — no custom Vite/Rollup build is used.
- Every pull request that changes the published library's public API or behavior MUST include a changeset file, per [[../adr/release-versioning-strategy.md]].
...
- **Publishing changes to the library without a changeset**
  - Consequence: the release tooling has nothing to base the next version bump/changelog entry on, reintroducing the manual-discipline risk [[../adr/release-versioning-strategy.md]] was chosen to avoid
