...
# Resolution
**Canonical — no resolver.** Delta-conflict-analysis [Finding 4](skills/angular/architecture/delta-conflict-analysis.md#findings) is now fully closed: `solution-authentication` carries its own `Implementation/GlobalStore/shared-state.project.extend.md` (previously the `auth` slice was registered only in prose). The example's `store.config.ts` registers `connectivity`, `notifications` and `auth` alongside the root store; `store.config.spec.ts` asserts all three are wired.
