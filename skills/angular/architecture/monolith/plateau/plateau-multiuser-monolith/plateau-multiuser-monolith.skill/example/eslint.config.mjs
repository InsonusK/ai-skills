      ...
      '@nx/enforce-module-boundaries': [
        'error',
        {
          ...
          depConstraints: [
            ...
            {
              // + type:store / type:data-access (both scope:shared) — libs/shared/
              // logging's BackendLogSink (VP6) sends batches through the base
              // http-core service and reads the auth slice's token via the
              // interceptor path. The scope:shared → scope:shared rule keeps this
              // to the shared primitives only.
              sourceTag: 'type:util',
              onlyDependOnLibsWithTags: [
                'type:util',
                'type:data-access',
