...
describe('provideGlobalStore', () => {
  ...
  it('wires the root store with the connectivity + notifications + auth slices registered', () => {
    const store = TestBed.inject(Store);
    let snapshot: Record<string, unknown> = {};
    store.subscribe((s) => (snapshot = s as Record<string, unknown>)).unsubscribe();
    expect(Object.keys(snapshot).sort()).toEqual(['auth', 'connectivity', 'notifications']);
