---
name: plateau-persisted-state-monolith--class-mutation-queue-service
description: The Dexie-backed mutation queue service — enqueue (with idempotency key), per-feature FIFO reads, a liveQuery observable for pending-sync UI, and markSynced — persisted-state-monolith plateau
domain: skill
type: template
