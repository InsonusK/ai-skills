---
name: plateau-persisted-state-monolith--project-shared-http-core
description: Base HTTP service shared by every feature's Client (base URL, timeout, retry) plus the shared OfflineTransportError every Client throws on a network-level failure — persisted-state-monolith plateau
domain: skill
type: template
