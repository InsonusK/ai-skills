---
name: plateau-persisted-state-monolith--project-feature-feature
description: Generic template for any {feature}/feature lib — routed, presentational + container components (including Signal Forms) for one feature, its feature-level Signal Store, its own root-relative routes with loadComponent sub-splitting, a VP5 offline-sync replay handler in route providers, and VP7 permission guards (`requirePermission`) + permission-gated UI (`*hasPermission`), unit/component tested via Vitest + Testing Library. — persisted-state-monolith plateau
domain: skill
type: template
...
/libs/{feature}
  /feature
    /src
      /lib
        [{feature}.store.ts](./classes/plateau-persisted-state-monolith--class-feature-store.skill.md)
        {feature}.store.spec.ts
        [{feature}-draft.store.ts](./classes/plateau-persisted-state-monolith--class-feature-draft-store.skill.md)   <- optional (VP8): signalStore + withPersistedDraft, editable fields only, cleared on submit
        {feature}-draft.store.spec.ts
        [{feature}.routes.ts](./classes/plateau-persisted-state-monolith--class-feature-routes.skill.md)   <- loadComponent split (VP1); parent route `providers: [provide{Feature}OfflineSync()]` (VP5); `canActivate: [requirePermission('...')]` on a restricted sub-route (VP7)
        {feature}.routes.spec.ts   <- asserts no self-set preload flag; split sub-route is loadComponent; route registers the offline-sync provider; guarded route redirects to /forbidden without the permission
        ...
        /{form-name}
          [{form-name}.component.ts](./classes/plateau-persisted-state-monolith--class-form-component.skill.md)
...
| ------------------ | ----------- | -------------- |
| `{feature}.store.ts` | Feature-level NgRx Signal Store. Owns all state and derived data specific to this feature. | [[skills/angular/architecture/monolith/plateau/plateau-persisted-state-monolith/structure/feature-feature/classes/plateau-persisted-state-monolith--class-feature-store.skill\|class-feature-store]] |
| `{feature}.store.spec.ts` | Vitest unit test faking the feature's Facade. | [[skills/angular/architecture/monolith/plateau/plateau-persisted-state-monolith/structure/feature-feature/classes/plateau-persisted-state-monolith--class-feature-store.skill\|class-feature-store]] |
| `{feature}.routes.ts` | Feature's own root-relative `Routes` array, exported from `index.ts`. Heavy/rare sub-routes are split via `loadComponent`; no route sets `data: { preload: true }`. | [[skills/angular/architecture/monolith/plateau/plateau-persisted-state-monolith/structure/feature-feature/classes/plateau-persisted-state-monolith--class-feature-routes.skill\|class-feature-routes]] |
| `{feature}.routes.spec.ts` | Vitest guard: no route self-sets `preload`; the heavy sub-route uses `loadComponent`, the main path does not. | [[skills/angular/architecture/monolith/plateau/plateau-persisted-state-monolith/structure/feature-feature/classes/plateau-persisted-state-monolith--class-feature-routes.skill\|class-feature-routes]] |
| `{form-name}.component.ts` (+ optional `{form-name}.form.ts`) | Any component in this feature that renders a form. | [[skills/angular/architecture/monolith/plateau/plateau-persisted-state-monolith/structure/feature-feature/classes/plateau-persisted-state-monolith--class-form-component.skill\|class-form-component]] |
| `{form-name}.component.spec.ts` | Testing Library component test, faking the component's Signal Store. | [[skills/angular/architecture/monolith/plateau/plateau-persisted-state-monolith/structure/feature-feature/classes/plateau-persisted-state-monolith--class-form-component.skill\|class-form-component]] |
| `{feature}-draft.store.ts` | Optional (VP8). `signalStore({ providedIn: 'root' })` + `withPersistedDraft`, holding only the form's editable fields; `patch` / `clear`. | [[skills/angular/architecture/monolith/plateau/plateau-persisted-state-monolith/structure/feature-feature/classes/plateau-persisted-state-monolith--class-feature-draft-store.skill\|class-feature-draft-store]] |
...
- The `requirePermission` factory or the `HasPermissionDirective` — the feature only *attaches* them; both are defined in `libs/shared/auth-ui`
...
- **VP7** — a route this feature restricts must carry `canActivate: [requirePermission('<permission-string>')]` attached in `{feature}.routes.ts` (add `canMatch` when the route must be invisible). A control gated in a component uses `*hasPermission="'<permission-string>'"`. Both take a permission string, never a role name, and both come from `@org/shared-auth-ui` — the feature never redefines either.
- **VP7** — a component/store in this feature must never read `currentUser`/`permissions` to build its own visibility logic; it defers to `*hasPermission` (view) or `requirePermission` (route). Feature code never persists or forwards the access token.
- **VP8** — a persisted form draft lives in a **dedicated** `{Feature}DraftStore` (`withPersistedDraft` from `@org/shared-state`), never on the feature's main list/detail Signal Store; the allow-list is exactly the form's editable fields; the draft is cleared on submit success. The `key` is namespaced (`app:{feature}:draft`).
...
- [ ] Every permission-restricted route uses `requirePermission(...)` from `@org/shared-auth-ui`, attached in `{feature}.routes.ts`; no guard or `HasPermissionDirective` is redefined in the feature
...
- [ ] Any persisted form draft is a dedicated `{Feature}DraftStore` with a field-only allow-list, cleared on submit success — never `withPersistedDraft` on the main store
