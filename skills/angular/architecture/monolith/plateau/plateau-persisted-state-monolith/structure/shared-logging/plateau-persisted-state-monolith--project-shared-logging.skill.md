---
name: plateau-persisted-state-monolith--project-shared-logging
description: LoggerService on the LOG_SINKS extension point, with ConsoleLogSink plus (VP6) BackendLogSink (batched warn/error/report), a bounded IndexedDB LogRetryQueue, and LoggerService.report() — persisted-state-monolith plateau
domain: skill
type: template
...
| ------------------ | ----------- | -------------- |
| backend-log-sink.ts | The second `LogSink`. Only warn/error/report; batched; sendBeacon on unload; `LogRetryQueue` on failure. | [[skills/angular/architecture/monolith/plateau/plateau-persisted-state-monolith/structure/shared-logging/classes/plateau-persisted-state-monolith--class-backend-log-sink.skill.md\|class-backend-log-sink]] |
| log-retry-queue.ts | Bounded IndexedDB retry queue. | [[skills/angular/architecture/monolith/plateau/plateau-persisted-state-monolith/structure/shared-logging/classes/plateau-persisted-state-monolith--class-log-retry-queue.skill.md\|class-log-retry-queue]] |
| logger.service.ts | `report()` (always to the backend); `LOG_SINKS` factory adds `BackendLogSink`. | [[skills/angular/architecture/monolith/plateau/plateau-persisted-state-monolith/structure/shared-logging/classes/plateau-persisted-state-monolith--class-logger-service.skill.md\|class-logger-service]] |
...
/libs/shared/logging
  /src
    /lib
      [logger.service.ts](./classes/plateau-persisted-state-monolith--class-logger-service.skill.md)
      log-sink.ts
      console-log-sink.ts
      log-level.token.ts
    index.ts
...
| `Directory\|file` | Description | Pattern skill |
| ------------------ | ----------- | -------------- |
| logger.service.ts | Public API: `debug/info/warn/error/report(message, context?)`, plus `forFeature(name)`. Forwards each entry to every registered `LogSink`, after checking it against the configured minimum level. | [[skills/angular/architecture/monolith/plateau/plateau-persisted-state-monolith/structure/shared-logging/classes/plateau-persisted-state-monolith--class-logger-service.skill\|class-logger-service]] |
