---
name: plateau-offline-full-monolith--project-shared-logging
description: LoggerService with a pluggable LogSink extension point — ConsoleLogSink is the only sink registered at this plateau. No backend delivery yet. — offline-full-monolith plateau
domain: skill
type: template
...
> No backend log delivery yet — `BackendLogSink` and its retry queue arrive with [[skills/angular/architecture/monolith/plateau/plateau-multiuser-monolith/plateau-multiuser-monolith.skill/plateau-multiuser-monolith.skill.md|multiuser-monolith]].
...
/libs/shared/logging
  /src
    /lib
      [logger.service.ts](./classes/plateau-offline-full-monolith--class-logger-service.skill.md)
      log-sink.ts
      console-log-sink.ts
      log-level.token.ts
    index.ts
...
| ------------------ | ----------- | -------------- |
| logger.service.ts | Public API: `debug/info/warn/error/report(message, context?)`, plus `forFeature(name)`. Forwards each entry to every registered `LogSink`, after checking it against the configured minimum level. | [[skills/angular/architecture/monolith/plateau/plateau-offline-full-monolith/structure/shared-logging/classes/plateau-offline-full-monolith--class-logger-service.skill\|class-logger-service]] |
