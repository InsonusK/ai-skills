...
Built per [[skills/common-workflow/architecture/design/plateau-map/feature-map-create.skill/feature-map-create.skill.md|feature-map-create]]. The `platform-host/` catalog composes this one; `embeddable-app/` and `design-system/` are independent — see [[skills/angular/architecture/README.md|the catalog overview]].
...
| PersistedState | An explicit, allow-listed subset of NgRx state — a `GlobalStore` slice through a per-slice persistence metaReducer, or a feature Signal Store through `withPersistedDraft()` — synced to `localStorage` / `sessionStorage` / IndexedDB so it survives a browser session (preferences, form drafts, filter state). `assertPersistable()` rejects any `SENSITIVE_STATE_KEYS` field; the `auth` slice is never a target. Requires `GlobalStore` (or the feature Signal Store tier). | false |
...
Proposed as flagged, no `Realized by`:
...
`PersistedState` was in this table last cycle; it is now **monolith VP8**, a Features-table row above, realized by [[skills/angular/architecture/solutions/solution-persisted-state.skill/solution-persisted-state.skill.md|solution-persisted-state]] — see the [Variability Map](../variability-map.md).

## Open questions on V1

**All resolved.** Recorded during Stage 1; each was resolved by the owner across Stages 2–4. The resolution is what this model now reflects.

1. **`solution-repository-structure`'s "split every feature into `feature` + `data-access` from the start" is wrong once `BackendDataAccess` is a VP** — a no-backend feature has no `data-access` lib. **Resolved:** `solution-repository-structure` was modified — a feature is a `feature` lib; a `data-access` lib is added by `BackendDataAccess` for features with server data. ADR [[skills/angular/architecture/solutions/solution-repository-structure.skill/adr/feature-lib-split-conditional-on-backend-data-access.md|feature-lib-split-conditional-on-backend-data-access]].
2. **`ComponentTesting` common or variable?** First principles say visual-regression + a11y *automation* is a quality gate, not a functional necessity (→ variable, like `PerformanceTunedRouting`); V1 and dotnet's equivalent call put all build/test gates in the common baseline. **Resolved (working default): common** — the build treats it as common (`solution-ui-testing` in every monolith plateau). Left flagged in the Features table: a later ruling to variable would add a VP9 (Yes/No, realized by `solution-ui-testing`).
3. **`solution-ui-testing`'s `depends_on` is family-crossed** — it declared `design-system-structure` + `design-system-components`, absent from `monolith/`. **Resolved:** split during delta-conflict-detection (Finding 1) into a monolith-side [[skills/angular/architecture/solutions/solution-ui-testing.skill/solution-ui-testing.skill.md|solution-ui-testing]] (deps: `repository-structure` + `app-testing` + `forms`) and a design-system-side [[skills/angular/architecture/solutions/solution-design-system-ui-testing.skill/solution-design-system-ui-testing.skill.md|solution-design-system-ui-testing]].
4. **`libs/shared/state` ownership.** **Resolved (owner, Stage 1):** `GlobalStore` is a **VP**, realized by [[skills/angular/architecture/solutions/solution-global-store.skill/solution-global-store.skill.md|solution-global-store]] (carved from V1 `solution-state-management`: the tiering rule + feature Signal Store pattern stay common as `StateTieringPolicy`; `libs/shared/state` + classical NgRx become the `GlobalStore` VP). `Authentication` / `OfflineReadResilience` / `OfflineWriteQueue` / `PersistedState` `require GlobalStore` and add their slices to it — the dotnet v3.1 "VP5/6/7 require VP2 (Persistence)" pattern, recorded in ADR [[skills/angular/architecture/solutions/solution-global-store.skill/adr/stateful-vps-build-on-the-global-store.md|stateful-vps-build-on-the-global-store]]. The V1 `auth.store.ts` worked example moved into `Authentication`.
5. **The `notifications` slice** `OfflineWriteQueue` needs for conflict messages was a "worked example" stub in `solution-state-management` and unowned. **Resolved:** `solution-offline-sync` (v3.1) creates it.
6. **`solution-lazy-loading-routing` → rename `PerformanceTunedRouting`** — the baseline `HierarchicalRouting` already lazy-loads via `loadChildren`; what varies is selective preloading + `loadComponent` discipline + enforced budgets. **Resolved:** renamed to [[skills/angular/architecture/solutions/solution-performance-tuned-routing.skill/solution-performance-tuned-routing.skill.md|solution-performance-tuned-routing]].
7. **`apps/platform-shell` name** — "platform-shell" is a platform-ism in a plain monolith. **Resolved:** kept the V1 name (a rename is ~50 files of churn for little gain).

## Out of scope

- **Aspirational candidates have no `Realized by`** — the rows in the Aspirational-candidates table are not Features-table rows; their variants/constraints come from reasoning, not working solutions, until one is authored.
- **Plateau Components** — optional cross-cutting capabilities attached at the composition root *after* composition (product analytics / RUM is the likely candidate, per the Telemetry aspirational row) — are out of scope here; they are built per [[skills/common-workflow/architecture/design/plateau-component-create.skill/plateau-component-create.skill.md|plateau-component-create]], not modeled as features.
