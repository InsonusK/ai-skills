...
- **Plateaus 7–8 (`plateau-platform-host`, `plateau-embeddable-app`): not started.** Both **limited** (federation smoke test, not full apps). `platform-host` composes `plateau-multiuser-monolith` cross-catalog via `parent_plateaus`; `embeddable-app` is from-scratch. The monolith chain + design-system are complete (6 of 8 plateaus).
