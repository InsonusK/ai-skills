...
- **Plateaus 6–8 (`plateau-design-system`, `plateau-platform-host`, `plateau-embeddable-app`): not started.** The monolith chain is complete. `design-system` is from-scratch (FULL); `platform-host` + `embeddable-app` are limited (federation smoke test), cross-catalog `parent_plateaus: [plateau-multiuser-monolith]`.
