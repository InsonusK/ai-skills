---
name: plateau-multi-tenant-design-system--class-component-name-style-snapshot-spec
description: Generic computed-CSS-property snapshot spec for a ds-* component at spec/{component}.style-snapshot.spec.ts — run paired with the visual spec so a failing pixel diff can be explained, not just observed — multi-tenant-design-system plateau
domain: skill
type: template
