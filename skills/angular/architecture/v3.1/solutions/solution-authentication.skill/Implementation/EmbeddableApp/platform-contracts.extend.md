...
No new directories in this monorepo — `@platform/contracts` is published from its own separate repository, per `solution-platform-embeddability`. This extension describes the contract addition conceptually; the actual package's own repository owns the concrete implementation.
