...
This extends the routes pattern from [[skills/angular/architecture/v3.1/solutions/solution-app-routing.skill/Implementation/FeatureRoutes/{Feature}.project.extend/{feature}.routes.ts.create]] It applies inside any `libs/{feature}/feature` project, once that feature itself is already lazily loaded as a whole via `loadChildren` at its mounting point.
...
## MUST
- A feature's own chunk MUST declare a per-chunk bundle budget (see [[skills/angular/architecture/v3.1/solutions/solution-lazy-loading-routing.skill/Implementation/Repository.extend#MUST|Repository]]); a sub-route split via `loadComponent` gets its own separate budget.
