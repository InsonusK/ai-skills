...
## MUST
- Every custom component in this library must consume `--mat-sys-*` tokens directly for colors/typography/elevation — it must never define a `--ds-color-primary`-style alias for something Material already models, per [[skills/angular/architecture/v3.1/solutions/solution-design-system-tokens.skill/adr/token-consumption-strategy.md|token-consumption-strategy]].
...
- Every color token (both `--mat-sys-*`, generated by Material, and this library's own `--ds-*` additions) must use `light-dark()`, per [[skills/angular/architecture/v3.1/solutions/solution-design-system-tokens.skill/adr/light-dark-mode-strategy.md|light-dark-mode-strategy]].
- Token values must only be overridden via Angular Material's Sass override mixins (`mat.theme-overrides`, component-specific override mixins) — components must never hand-set `--mat-*` custom properties directly in raw CSS.

- Never introduce a palette-swapping or per-tenant theming mechanism — a single fixed brand palette is used everywhere, per [[skills/angular/architecture/v3.1/solutions/solution-design-system-tokens.skill/adr/brand-theming-scope.md|brand-theming-scope]].
# Unittest TestCases
