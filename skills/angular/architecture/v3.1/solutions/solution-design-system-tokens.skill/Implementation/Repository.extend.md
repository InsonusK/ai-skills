...
## MUST
- Every custom component in this library MUST consume `--mat-sys-*` tokens directly for colors/typography/elevation — it MUST NOT define a `--ds-color-primary`-style alias for something Material already models, per [[skills/angular/architecture/v3.1/solutions/solution-design-system-tokens.skill/adr/token-consumption-strategy.md|token-consumption-strategy]].
