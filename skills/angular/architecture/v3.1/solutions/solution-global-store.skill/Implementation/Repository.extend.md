...
## SHOULD
- **Adding a feature-specific slice to `libs/shared/state` "because it might be needed elsewhere later"** — Consequence: erodes the tiering rule this solution exists to enforce, and turns the shared state lib into a dumping ground — Instead: keep the slice in the owning feature's Signal Store until a second, unrelated feature genuinely needs to read it
