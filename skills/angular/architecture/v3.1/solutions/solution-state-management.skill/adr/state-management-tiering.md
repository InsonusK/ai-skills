...
- **Feature-level state** (the data and UI state owned by one feature, e.g. the list of orders being managed): an NgRx Signal Store colocated in that feature's `feature` lib (see `solution-repository-structure`'s `{feature}/feature` structure).
...
- Feature-level Signal Stores stay colocated with the feature that owns them, consistent with `solution-repository-structure`'s module-boundary rules
