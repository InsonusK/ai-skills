...
| /src/styles.scss | Imports the design system's `theme.scss`/`custom-tokens.scss`, used when this app is run standalone during local development — per [[skills/angular/architecture/v3.1/solutions/solution-design-system-application.skill/adr/theme-application-scope.md|theme-application-scope]], this import is redundant (but harmless) once the app is mounted inside the platform in production. |
