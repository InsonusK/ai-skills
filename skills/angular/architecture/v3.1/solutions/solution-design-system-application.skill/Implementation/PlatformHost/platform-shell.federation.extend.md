...
| --------------- | ----------- |
| /apps/platform-shell/federation.config.ts | Extended: declares the design system as a shared dependency with `singleton: true`, `strictVersion: false`, and the platform's currently targeted version range, per [[skills/angular/architecture/v3.1/solutions/solution-design-system-application.skill/adr/design-system-version-negotiation.md|design-system-version-negotiation]]. |
| /apps/platform-shell/src/styles.scss | Imports the design system's `theme.scss` and `custom-tokens.scss` at the root, per [[skills/angular/architecture/v3.1/solutions/solution-design-system-application.skill/adr/theme-application-scope.md|theme-application-scope]] — the only production consumer required to do so. |

# Rules

## MUST
- `federation.config.ts` MUST declare the design system with `singleton: true` and `strictVersion: false` — never `strictVersion: true`, per [[skills/angular/architecture/v3.1/solutions/solution-design-system-application.skill/adr/design-system-version-negotiation.md|design-system-version-negotiation]].
