...
| /libs/{feature}/data-access | HTTP calls, DTO-to-domain-model mapping, and facade for one feature. Consumed only by that feature's `feature` lib (see `solution-api-http-layer` for the internal shape of this lib). |
