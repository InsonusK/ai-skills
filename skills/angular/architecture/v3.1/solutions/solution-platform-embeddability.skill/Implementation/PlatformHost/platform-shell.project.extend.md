...
| /remote-registry/remote-registry.service.ts | Runtime service that resolves the list of available embeddable apps and their `remoteEntry` URLs from configuration (see [[skills/angular/architecture/v3.1/solutions/solution-platform-embeddability.skill/Implementation/PlatformHost/platform-shell.project.extend/remote-registry.service.ts.create]]) |
