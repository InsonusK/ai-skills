...
Built per [[skills/dotnet/architecture/v3.1/design/feature-map-create.skill/feature-map-create.skill.md|feature-map-create]]. The `platform/` catalog composes this one; `design-system/` is independent — see [[skills/angular/architecture/v3.1/README.md|the catalog overview]].
...
- `libs/shared/state` (the global NgRx store) — **not owned by any single feature**: it is a shared artifact created on demand by whichever of `Authentication` / `OfflineReadResilience` / `OfflineWriteQueue` is composed first (tracked in [[skills/angular/architecture/v3.1/delta-conflict-analysis.md|delta-conflict-detection]], the way dotnet v3.1's `App.Infrastructure` is created by the first persistence solution). The V1 `auth.store.ts` worked example in `solution-state-management` moves into `Authentication`.
...
`OfflineReadResilience`, `BackendLogDelivery`, and `Authentication` are drawn as children of `BackendDataAccess` — each needs the HTTP layer (a Client to throw `OfflineTransportError` and API GETs to cache; `http-core` to send log batches; the interceptor / silent-refresh / login round trips), so their dependency is the parent-child edge, not a separate `Requires`. The one cross-tree `Requires` is `BackendLogDelivery` → `ConsoleLogging` (it registers a second `LogSink` and adds `LoggerService.report()`).
...
| StateTieringPolicy | The *rule* for where state lives: component `signal()` → feature NgRx Signal Store (colocated in `libs/{feature}/feature`) → global classical NgRx slice in `libs/shared/state`. Promoted upward only when a second unrelated consumer needs it; a feature never caches global state locally. The rule is common; **the global store is not materialized until a feature needs it**. | true |
...
- **`libs/shared/state`** is a shared on-demand artifact, not a feature (no independent Yes/No — it exists iff at least one of `Authentication` / `OfflineReadResilience` / `OfflineWriteQueue` is present).
...
4. **`libs/shared/state` ownership** — no V1 solution unambiguously owns the global store; `solution-state-management` `creates` it but only with a *worked-example* `auth.store.ts`. **Working hypothesis: the global-store skeleton is a shared on-demand artifact** created by the first of `Authentication` / `OfflineReadResilience` (connectivity slice) / `OfflineWriteQueue` (notifications slice); `solution-state-management` keeps only the tiering rule + the feature Signal Store pattern.
...
- **Aspirational features have no `Realized by`** — variants/constraints come from the diagram, not working solutions, until authored.
- **The federation / platform world is a separate catalog** — `platform/` composes this model; `FederationHost`, `SessionContract` publication, remotes, `@platform/contracts` are all there, not here.
