...
`PersistedState` was in this table last cycle; it is now **monolith VP8**, a Features-table row above, realized by [[skills/angular/architecture/v3.1/solutions/solution-persisted-state.skill/solution-persisted-state.skill.md|solution-persisted-state]] — see the [Variability Map](../variability-map.md).
...
1. **`solution-repository-structure`'s "split every feature into `feature` + `data-access` from the start" is wrong once `BackendDataAccess` is a VP** — a no-backend feature has no `data-access` lib. **Resolved:** `solution-repository-structure` was modified — a feature is a `feature` lib; a `data-access` lib is added by `BackendDataAccess` for features with server data. ADR [[skills/angular/architecture/v3.1/solutions/solution-repository-structure.skill/adr/feature-lib-split-conditional-on-backend-data-access.md|feature-lib-split-conditional-on-backend-data-access]].
...
3. **`solution-ui-testing`'s `depends_on` is family-crossed** — it declared `design-system-structure` + `design-system-components`, absent from `monolith/`. **Resolved:** split during delta-conflict-detection (Finding 1) into a monolith-side [[skills/angular/architecture/v3.1/solutions/solution-ui-testing.skill/solution-ui-testing.skill.md|solution-ui-testing]] (deps: `repository-structure` + `app-testing` + `forms`) and a design-system-side [[skills/angular/architecture/v3.1/solutions/solution-design-system-ui-testing.skill/solution-design-system-ui-testing.skill.md|solution-design-system-ui-testing]].
4. **`libs/shared/state` ownership.** **Resolved (owner, Stage 1):** `GlobalStore` is a **VP**, realized by [[skills/angular/architecture/v3.1/solutions/solution-global-store.skill/solution-global-store.skill.md|solution-global-store]] (carved from V1 `solution-state-management`: the tiering rule + feature Signal Store pattern stay common as `StateTieringPolicy`; `libs/shared/state` + classical NgRx become the `GlobalStore` VP). `Authentication` / `OfflineReadResilience` / `OfflineWriteQueue` / `PersistedState` `require GlobalStore` and add their slices to it — the dotnet v3.1 "VP5/6/7 require VP2 (Persistence)" pattern, recorded in ADR [[skills/angular/architecture/v3.1/solutions/solution-global-store.skill/adr/stateful-vps-build-on-the-global-store.md|stateful-vps-build-on-the-global-store]]. The V1 `auth.store.ts` worked example moved into `Authentication`.
