---
name: plateau-async-monolith--class-component-name-component-spec
description: Generic pattern for a component test under spec/, using Angular Testing Library and interacting through the rendered DOM — a component is tested purely on its own input()/output()/model() surface, independent of whatever business logic (if any) sits around it — async-monolith plateau
domain: skill
type: template
