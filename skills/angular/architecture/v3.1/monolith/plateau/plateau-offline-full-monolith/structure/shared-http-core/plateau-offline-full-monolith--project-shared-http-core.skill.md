---
name: plateau-offline-full-monolith--project-shared-http-core
description: Base HTTP service shared by every feature's Client (base URL, timeout, retry) plus the shared OfflineTransportError every Client throws on a network-level failure — offline-full-monolith plateau
domain: skill
type: template
