...
> **Fifth and last plateau of the monolith chain — and [`plateau-platform-host`](skills/angular/architecture/v3.1/platform-host/variability-map.md)'s parent.** Composes [[skills/angular/architecture/v3.1/monolith/plateau/plateau-offline-full-monolith/structure/plateau-offline-full-monolith--repo-offline-full-monolith.skill.md|plateau-offline-full-monolith]] (online + VP1 + VP4 + VP5) and adds **two** solutions: [[skills/angular/architecture/v3.1/solutions/solution-logging-global.skill/solution-logging-global.skill.md|solution-logging-global]] (**VP6 — BackendLogDelivery**) and [[skills/angular/architecture/v3.1/solutions/solution-authentication.skill/solution-authentication.skill.md|solution-authentication]] (**VP7 — Authentication**). VP1–VP7 = Yes; VP8 (PersistedState) aspirational. One **new** Nx project — `libs/shared/auth-ui` (`type:store`): the `*hasPermission` directive, the `requirePermission` guard factory, the login form + forbidden page. VP6 adds `BackendLogSink` + a bounded IndexedDB `LogRetryQueue` + `LoggerService.report()` to `libs/shared/logging`, and a `GlobalErrorHandler` to `apps/platform-shell`. VP7 adds the `auth` slice (in-memory token, silent refresh) + the `authInterceptor` to `libs/shared/state`, and bootstrap silent-refresh + interceptor registration to `apps/platform-shell`. Still one deployable unit, no Module Federation (that is `platform-host`). The [[skills/angular/architecture/v3.1/design-system/plateau/plateau-design-system/plateau-design-system.skill/plateau-design-system.skill.md|design-system]] npm package remains a plain dependency of `apps/platform-shell`.
...
| /apps/platform-shell-e2e | [[skills/angular/architecture/v3.1/monolith/plateau/plateau-multiuser-monolith/structure/platform-shell-e2e/plateau-multiuser-monolith--project-platform-shell-e2e.skill\|project-platform-shell-e2e]] | Playwright end-to-end scenario specs against the real built application. |
...
| /libs/shared/ui | [[skills/angular/architecture/v3.1/monolith/plateau/plateau-multiuser-monolith/structure/shared-ui/plateau-multiuser-monolith--project-shared-ui.skill\|project-shared-ui]] | Reusable, app-specific UI composed from design-system primitives. |
| /libs/shared/util | [[skills/angular/architecture/v3.1/monolith/plateau/plateau-multiuser-monolith/structure/shared-util/plateau-multiuser-monolith--project-shared-util.skill\|project-shared-util]] | Framework-agnostic pure helpers shared across features. |
...
- Feature code must never read `navigator.onLine` or the Dexie table directly — `selectIsOnline` for connectivity, `MutationQueueService.pendingForFeature$` for pending counts.
...
- Never let a feature build its own local offline / pending indicator — mount the shared `OfflineBannerComponent` (shell) and `PendingSyncIndicatorComponent` (per feature).
...
- [ ] WHEN a mutation is enqueued and the app reloads THEN
  - [ ] it persists (Dexie/IndexedDB) and `pendingForFeature$` still reports it
