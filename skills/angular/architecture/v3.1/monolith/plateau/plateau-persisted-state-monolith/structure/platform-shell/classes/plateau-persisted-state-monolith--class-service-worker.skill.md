---
name: plateau-persisted-state-monolith--class-service-worker
description: The Workbox service worker for apps/platform-shell — four content-type routing rules, testable predicates in sw-routes.ts, and the sw-build.mjs Nx build step — persisted-state-monolith plateau
domain: skill
type: template
