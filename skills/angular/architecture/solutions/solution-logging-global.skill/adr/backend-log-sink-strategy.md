...
- A bounded, persisted retry queue means a transient network outage does not silently lose error reports — but the three independent bounds (count/age/size) keep the queue from growing unbounded if the backend is unreachable for an extended period, without requiring the full durable offline-sync machinery the future `solution-offline-sync` will build for business data
