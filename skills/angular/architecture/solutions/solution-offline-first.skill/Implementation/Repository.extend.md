...
## MUST
- The service worker MUST be generated via Workbox's programmatic build API, integrated into the Nx build pipeline — not via a webpack-specific plugin, and not via `@angular/service-worker` (ngsw), per [[skills/angular/architecture/solutions/solution-offline-first.skill/adr/service-worker-mechanism]].
...
- Auth endpoints and every non-GET request MUST be configured as `network-only` in the service worker's routing rules — never cached, per [[skills/angular/architecture/solutions/solution-offline-first.skill/adr/caching-strategy-per-content-type]].
