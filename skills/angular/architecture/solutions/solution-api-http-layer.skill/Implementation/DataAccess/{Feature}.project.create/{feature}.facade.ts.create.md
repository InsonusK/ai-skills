...
- Any error the Facade lets through to its caller MUST be one of this feature's typed domain errors (from the Client, optionally re-wrapped with business context) — never a raw `HttpErrorResponse`, per [[skills/angular/architecture/solutions/solution-api-http-layer.skill/adr/error-handling-strategy]].
