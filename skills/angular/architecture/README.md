...
| --- | --- | --- | --- | --- |
| [`monolith/`](mo[`monolith/`](skills/angular/architecture/monolith/feature/feature-model.md)oyable Angular web application | Nx workspace: shell + feature libs + routing + testing | — | online-monolith, async-monolith, offline-monolith, monitored-app, multiuser-app |
| [`platform-host/`[`platform-host/`](skills/angular/architecture/platform-host/feature/feature-model.md)f a federation system + the `@platform/contracts` package | a `monolith/` + federation host config + contracts package | `monolith/` (a host **is** a monolith, via `parent_plateaus`) | platform-monolith |
| [`embeddable-app/`[`embeddable-app/`](skills/angular/architecture/embeddable-app/feature/feature-model.md)an independently deployed app a host loads at runtime | minimal: `remoteEntry` + exposed module + shared singletons (any tooling) | — (optionally a `monolith/` if `RemoteInternalArchitecture`) | embeddable-app |
| [`design-system/`]([`design-system/`](skills/angular/architecture/design-system/feature/feature-model.md) versioned component-library npm package | Angular CLI multi-project: library + demo | — | design-system |
