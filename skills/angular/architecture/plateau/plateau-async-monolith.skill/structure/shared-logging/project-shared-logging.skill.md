...
| ------------------ | ----------- | -------------- |
| logger.service.ts | Public API: `debug/info/warn/error/report(message, context?)`, plus `forFeature(name)`. Forwards each entry to every registered `LogSink`, after checking it against the configured minimum level. | [[skills/angular/architecture/plateau/plateau-async-monolith.skill/structure/shared-logging/classes/class-logger-service.skill\|class-logger-service]] |
