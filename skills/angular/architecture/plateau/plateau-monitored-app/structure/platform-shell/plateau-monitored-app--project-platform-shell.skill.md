---
name: plateau-monitored-app--project-platform-shell
description: The single deployable Angular application — composition root, routing, selective preloading, registered service worker, Native Federation dynamic host, and now a global ErrorHandler routing every uncaught exception to the backend log sink — monitored-app plateau
domain: skill
type: template
...
/apps/platform-shell
  /src
    ...
    /app
      ...
      app.routes.ts
      global-error-handler.ts         <- see [plateau-monitored-app--class-global-error-handler.skill.md](./classes/plateau-monitored-app--class-global-error-handler.skill.md)
      /remote-registry
        [remote-registry.service.ts](./classes/plateau-monitored-app--class-remote-registry-service.skill.md)
      /shell
        shell.component.ts            <- mounts a resolved remote via loadRemoteModule
    /preloading
      selective-preloading.strategy.ts
    sw-build.ts                       <- see [plateau-monitored-app--class-service-worker.skill.md](./classes/plateau-monitored-app--class-service-worker.skill.md)
    sw-src.ts                         <- see [plateau-monitored-app--class-service-worker.skill.md](./classes/plateau-monitored-app--class-service-worker.skill.md)
    main.ts
```
...
| app/global-error-handler.ts | Global `ErrorHandler` routing every uncaught exception through `LoggerService.error` | [[skills/angular/architecture/plateau/plateau-monitored-app/structure/platform-shell/classes/plateau-monitored-app--class-global-error-handler.skill\|class-global-error-handler]] |
| /remote-registry/remote-registry.service.ts | Runtime service resolving the list of available embeddable apps and their `remoteEntry` URLs from configuration | [[skills/angular/architecture/plateau/plateau-monitored-app/structure/platform-shell/classes/plateau-monitored-app--class-remote-registry-service.skill\|class-remote-registry-service]] |
...
| sw-build.ts / sw-src.ts | Workbox-generated service worker: app-shell precache, static-asset cache-first, API-read stale-while-revalidate, auth/mutation network-only, federated-remote stale-while-revalidate | [[skills/angular/architecture/plateau/plateau-monitored-app/structure/platform-shell/classes/plateau-monitored-app--class-service-worker.skill\|class-service-worker]] |
...
## MUST NOT
- [[skills/angular/architecture/plateau/plateau-monitored-app/structure/plateau-monitored-app--repo-monitored-app.skill#MUST NOT|repo-monitored-app]]
