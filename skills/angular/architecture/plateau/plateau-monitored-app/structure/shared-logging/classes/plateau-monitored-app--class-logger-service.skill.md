---
name: plateau-monitored-app--class-logger-service
description: Central logging entry point — structured debug/info/warn/error/report calls forwarded to every registered LogSink, with environment-based level filtering (report() always bypasses the filter) — monitored-app plateau
domain: skill
type: template
