...
> Second plateau in the main application's chain. Parent: [[skills/angular/architecture/plateau/online-monolith/plateau-online-monolith.skill.md|online-monolith]]. Next: [[skills/angular/architecture/plateau/offline-monolith/plateau-offline-monolith.skill.md|offline-monolith]]. This is the **"async-monolith"** milestone: feature chunks are lazy-loaded and selectively preloaded, and the app keeps working against cached GET data — with a visible offline indicator — when the network is unreliable. Auth/mutation endpoints stay strictly `network-only`; a genuinely offline mutation still fails immediately (durable queueing and replay arrive with [[skills/angular/architecture/plateau/offline-monolith/plateau-offline-monolith.skill.md|offline-monolith]]). Still no authentication, no Module Federation, no backend log delivery.
...
| ---------- | ------------- | ----------- |
| /apps/platform-shell | [[platform-shell/project-platform-shell.skill.md\|project-platform-shell.skill]] | Composition root: top-level routing, selective preloading strategy, service-worker registration. Still no business logic. |
| /apps/platform-shell-e2e | [[platform-shell-e2e/project-platform-shell-e2e.skill.md\|project-platform-shell-e2e.skill]] | Playwright end-to-end scenario specs, now including an offline-read scenario. |
| /libs/shared/ui | [[shared-ui/project-shared-ui.skill.md\|project-shared-ui.skill]] | Reusable, app-specific UI, now including the offline banner. |
...
| /libs/shared/state | [[shared-state/project-shared-state.skill.md\|project-shared-state.skill]] | Classical NgRx Store, now including the `connectivity` slice (`isOnline`). |
| /libs/shared/http-core | [[shared-http-core/project-shared-http-core.skill.md\|project-shared-http-core.skill]] | Base HTTP service; now also hosts the shared `OfflineTransportError` type. |
| /libs/shared/logging | [[shared-logging/project-shared-logging.skill.md\|project-shared-logging.skill]] | `LoggerService`, console-only — unchanged from `online-monolith`. |
| /libs/{feature}/feature | [[feature-feature/project-feature-feature.skill.md\|project-feature-feature.skill]] | Generic template: routed components split into per-route lazy chunks via `loadComponent`. |
| /libs/{feature}/data-access | [[feature-data-access/project-feature-data-access.skill.md\|project-feature-data-access.skill]] | Generic template: Facade/Client/Mapper/Errors, the Client now distinguishing an offline network failure from a server rejection. |
...
- All other rules from [[skills/angular/architecture/plateau/online-monolith/plateau-online-monolith.skill.md|online-monolith]] continue to apply unchanged.
