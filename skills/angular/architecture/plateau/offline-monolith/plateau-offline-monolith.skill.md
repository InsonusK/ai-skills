...
parent_plateau: "[[skills/angular/architecture/plateau/async-monolith/plateau-async-monolith.skill.md|async-monolith]]"
---

> Parent: [[skills/angular/architecture/plateau/async-monolith/plateau-async-monolith.skill.md|async-monolith]] (lazy-loaded routing plus read resilience — 2 solutions on top of `online-monolith`'s 7). This plateau adds `solution-offline-sync` on top, unchanged otherwise. Next: [[skills/angular/architecture/plateau/platform-monolith/plateau-platform-monolith.skill.md|platform-monolith]], where the monolith is split into independent, embeddable modules. Still no authentication (that arrives at [[skills/angular/architecture/plateau/multiuser-app/plateau-multiuser-app.skill.md|multiuser-app]], the last plateau), no Module Federation, no backend log delivery.
