...
| ------------------ | ----------- | -------------- |
| /auth | Real session lifecycle: login, silent refresh, logout, session expiry. Platform-host-side source of `SessionContract`. | [[classes/class-auth-store.skill.md\|class-auth-store.skill]] |
| /auth/auth.interceptor.ts | Attaches the in-memory access token to outgoing requests; triggers silent refresh on 401. | [[classes/class-auth-interceptor.skill.md\|class-auth-interceptor.skill]] |
