...
| ------------------ | ----------- | -------------- |
| logger.service.ts | Public API: `debug/info/warn/error/report(message, context?)`, plus `forFeature(name)`. Forwards each entry to every registered `LogSink`. | [[classes/class-logger-service.skill.md\|class-logger-service.skill]] |
...
| backend-log-sink.ts | Batches `warn`/`error`/`report()` entries and POSTs them to the backend. | [[classes/class-backend-log-sink.skill.md\|class-backend-log-sink.skill]] |
| log-retry-queue.ts | Durable, persisted queue that retries a failed batch send instead of dropping it. | [[classes/class-log-retry-queue.skill.md\|class-log-retry-queue.skill]] |
