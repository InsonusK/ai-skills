---
name: plateau-multiuser-app--class-logger-service
description: Central logging entry point — structured debug/info/warn/error/report calls forwarded to every registered LogSink, with environment-based level filtering (report() always bypasses the filter) — multiuser-app plateau
domain: skill
type: template
