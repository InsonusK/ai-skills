...
| ------------------ | ----------- | -------------- |
| mutation-queue.db.ts / mutation-queue.service.ts | Dexie schema and public API (`enqueue`, `pendingForFeature$`, `markSynced`) for the durable, per-feature-partitioned queue | [[skills/angular/architecture/plateau/plateau-multiuser-app.skill/structure/shared-offline-sync/classes/class-mutation-queue-service.skill\|class-mutation-queue-service]] |
| replay-orchestrator.ts | Triggered by `connectivity`'s `isOnline` transitioning to `true`; replays each feature partition independently and in parallel, FIFO within a partition | [[skills/angular/architecture/plateau/plateau-multiuser-app.skill/structure/shared-offline-sync/classes/class-replay-orchestrator.skill\|class-replay-orchestrator]] |
