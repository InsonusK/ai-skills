...
> Generic pattern, not tied to one concrete feature — any feature's own `{feature}.store.ts` follows this, substituting `{Feature}`/`{feature}` with the real feature name. Unchanged since [[skills/angular/architecture/plateau/foundation/plateau-foundation.skill.md|foundation]] — no solution in this plateau touches the feature store.
