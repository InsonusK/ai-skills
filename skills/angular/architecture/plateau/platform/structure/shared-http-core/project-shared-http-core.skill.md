...
- Give every feature's Client a single, consistent place for cross-cutting HTTP concerns
...
| offline-transport.error.ts | `OfflineTransportError` — the single, shared type every feature's Client throws when a request never reaches the server | — |
...
- DTO mapping — belongs in each feature's own Client/Mapper
- Auth token attachment — handled globally by `authInterceptor`
...
- `OfflineTransportError` MUST be defined once here, not redefined per feature.
...
- **Adding a feature-specific special case directly into `base-http.service.ts`**
  ...
  - Instead: keep this service generic
