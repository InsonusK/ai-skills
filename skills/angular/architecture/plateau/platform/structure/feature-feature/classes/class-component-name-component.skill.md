...
> Unchanged since [[skills/angular/architecture/plateau/offline-app/plateau-offline-app.skill.md|offline-app]]. Component-local state always uses a plain `signal()` — see [[../../repo-platform.skill.md|repo-platform]]'s cross-cutting conventions for that rule; it applies to every component in the workspace.
