...
export class BackendLogSink implements LogSink {
  ...
  private async flush(): Promise<void> {
    ...
    try {
      ...
      await this.retryQueue.enqueue(batch);
    }
  }
...
## MUST
- `BackendLogSink` MUST filter to `warn`/`error`/`report` entries only.
- On a failed flush, the batch MUST be handed to `LogRetryQueue.enqueue(...)`.
- The unload flush MUST use `navigator.sendBeacon`.
...
- **Sending each entry as its own HTTP request instead of buffering**
  ...
  - Instead: always buffer and flush on a timer/size threshold/unload
...
- [ ] A failed flush always results in the batch being enqueued via `LogRetryQueue`
...
- [ ] WHEN the page is unloaded with entries still buffered THEN
  - [ ] `navigator.sendBeacon` is called
