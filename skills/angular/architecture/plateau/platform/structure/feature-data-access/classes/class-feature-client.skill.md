...
description: Generic pattern for a feature's Client — the internal transport layer of its data-access lib, mapping DTOs and translating every transport failure (including offline) into a typed domain error
domain: skill
type: template
plateau: platform
...
- Never exported from the feature's `index.ts`
- A network-level failure (`status === 0`) is checked before any status-code-specific handling and throws the shared `OfflineTransportError`
...
- Every Client method's error handling MUST check for a network-level failure (`status === 0`) before any status-code-specific handling, and throw `OfflineTransportError` in that case.
- DTO ↔ domain model mapping MUST go through this feature's `{feature}.mapper.ts` functions.
- Every test in this file's spec MUST use `HttpTestingController`, and `httpTesting.verify()` MUST run in `afterEach`.
...
- **Treating a `status === 0` failure the same as any other server error**
  - Consequence: the Facade (and the offline-sync replay orchestrator) has no reliable way to tell "we're offline" apart from "the server actively rejected this"
  - Instead: always check for the network-level failure first
- **Using `HttpTestingController` inside a Facade or Signal Store test**
...
- [ ] Every HTTP call goes through `libs/shared/http-core`
...
- [ ] WHEN the backend returns a 409 conflict THEN
  - [ ] the Client throws `{Feature}ConflictError`
- [ ] WHEN the backend returns a successful response THEN
  - [ ] the Client returns a correctly mapped domain model
- [ ] WHEN a Client method's HTTP call fails with no response received THEN
  - [ ] it throws `OfflineTransportError`
- [ ] WHEN a Client method's HTTP call fails with an actual server response THEN
  - [ ] it throws that feature's own typed domain error
