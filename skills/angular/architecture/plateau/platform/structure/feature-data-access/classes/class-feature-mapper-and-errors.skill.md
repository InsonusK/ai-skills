...
> Generic pattern, not tied to one concrete feature. Covers `{feature}.mapper.ts` and `{feature}.errors.ts`.

# Goal

- Give every field conversion between DTO and domain model an explicit, reviewable, unit-testable function
- Give every feature a small, typed set of domain errors
...
- No inline field renaming anywhere else
...
export function orderDtoToModel(dto: OrderDto): Order {
  return { id: dto.id, quantity: dto.qty, createdAt: new Date(dto.created_at) };
}
...
- Every domain error thrown by this feature's Client or Facade MUST be one of the classes defined in `{feature}.errors.ts` (or the shared `OfflineTransportError`).
...
- **A mapper function silently dropping a DTO field with no domain equivalent, with no comment explaining why**
  ...
  - Instead: leave a short comment stating so
