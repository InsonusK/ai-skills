---
name: class-scenario-name-e2e-spec
description: Generic pattern for a Playwright end-to-end test in apps/platform-shell-e2e, exercising a real user scenario through a real browser
domain: skill
type: template
plateau: platform
artifact_type: test
...
> Generic pattern, not tied to one concrete scenario.

# Goal

- Verify a complete user-facing scenario through a real browser, across real routing (including lazy-loaded features and federated embeddable modules)
...
- E2E tests are reserved for a small number of critical, cross-cutting user journeys
...
## MUST
- E2E scenarios MUST navigate and interact exactly as a real user would — not by calling internal application APIs directly.
