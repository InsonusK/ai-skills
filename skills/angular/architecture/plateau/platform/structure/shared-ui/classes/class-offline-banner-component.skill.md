---
name: class-offline-banner-component
...
## MUST
- The banner MUST read `isOnline` from the shared `connectivity` slice.
- The banner MUST be mounted once, in `apps/platform-shell`.
...
- **A feature implementing its own local offline indicator instead of using this shared component**
  ...
  - Instead: mount `OfflineBannerComponent` once at the shell level
