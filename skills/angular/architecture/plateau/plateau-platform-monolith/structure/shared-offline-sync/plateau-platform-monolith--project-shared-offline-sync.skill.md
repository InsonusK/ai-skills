---
name: plateau-platform-monolith--project-shared-offline-sync
description: Dexie-backed mutation queue — durable, per-feature-partitioned storage for mutations attempted offline, plus the replay orchestrator that syncs them once connectivity is restored — platform-monolith plateau
domain: skill
type: template
...
/libs/shared/offline-sync
  /src
    /lib
      [mutation-queue.db.ts, mutation-queue.service.ts](./classes/plateau-platform-monolith--class-mutation-queue-service.skill.md)
      [replay-orchestrator.ts](./classes/plateau-platform-monolith--class-replay-orchestrator.skill.md)
    index.ts
```
...
| ------------------ | ----------- | -------------- |
| mutation-queue.db.ts / mutation-queue.service.ts | Dexie schema and public API (`enqueue`, `pendingForFeature$`, `markSynced`) for the durable, per-feature-partitioned queue | [[skills/angular/architecture/plateau/plateau-platform-monolith/structure/shared-offline-sync/classes/plateau-platform-monolith--class-mutation-queue-service.skill\|class-mutation-queue-service]] |
| replay-orchestrator.ts | Triggered by `connectivity`'s `isOnline` transitioning to `true`; replays each feature partition independently and in parallel, FIFO within a partition | [[skills/angular/architecture/plateau/plateau-platform-monolith/structure/shared-offline-sync/classes/plateau-platform-monolith--class-replay-orchestrator.skill\|class-replay-orchestrator]] |
