---
name: plateau-platform-monolith--class-feature-routes
description: Generic pattern for a feature's own routes — defined and exported relative to the feature's own root only, with per-route permission guards and loadComponent splitting where justified — platform-monolith plateau
domain: skill
type: template
...
export const ORDERS_ROUTES: Routes = [
  ...
  },
  {
    ...
    canActivate: [requirePermission('orders.delete')], // from libs/shared/auth-ui
...
## MUST
- `{feature}.routes.ts` paths MUST be defined relative to the feature's own root only — never including the feature's own name or an assumed mount prefix.
- `{FEATURE}_ROUTES` MUST be exported from `index.ts`.
- A feature's own chunk MUST declare a per-chunk bundle budget; a `loadComponent`-split sub-route gets its own separate budget.
...
- **Splitting every single sub-route into its own `loadComponent` chunk regardless of size**
  - Consequence: excessive number of tiny chunks, each paying its own network round-trip cost
...
- **A feature marking its own route `preload: true` "because it's important"**
  ...
  - Instead: the feature stays silent on preloading; the shell decides
