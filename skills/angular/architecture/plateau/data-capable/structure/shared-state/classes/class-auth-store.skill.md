...
> This is the base auth slice, created as the worked example of the global-state tier. Unchanged since [[skills/angular/architecture/plateau/foundation/plateau-foundation.skill.md|foundation]] — session lifecycle logic (login flow, silent refresh, permissions) is added by the future [[skills/angular/architecture/plateau/authenticated/plateau-authenticated.skill.md|authenticated]] plateau; this plateau establishes only the slice's shape.
