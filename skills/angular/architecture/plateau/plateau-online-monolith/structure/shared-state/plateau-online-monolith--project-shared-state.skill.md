...
description: Classical NgRx Store hosting global, cross-cutting state slices — currently an auth-slice skeleton (worked example of the global tier; no real session lifecycle yet) and a home for future notifications/offline-sync slices — online-monolith plateau
domain: skill
type: template
...
> No authentication yet — the auth slice below is `solution-state-management`'s own worked example of the global-state tier, not a real session lifecycle. That arrives in full with [[skills/angular/architecture/plateau/plateau-multiuser-app/plateau-multiuser-app.skill.md|multiuser-app]], the last plateau in this chain.
...
/libs/shared/state
  /src
    /lib
      /auth
        [auth.actions.ts, auth.reducer.ts, auth.effects.ts, auth.selectors.ts](./classes/plateau-online-monolith--class-auth-store.skill.md)
      /notifications
      /offline-sync
    index.ts
```
...
| ------------------ | ----------- | -------------- |
| /auth | Skeleton auth slice — worked example of the global-state tier. Real session lifecycle arrives with `multiuser-app`. | [[skills/angular/architecture/plateau/plateau-online-monolith/structure/shared-state/classes/plateau-online-monolith--class-auth-store.skill\|class-auth-store]] |
