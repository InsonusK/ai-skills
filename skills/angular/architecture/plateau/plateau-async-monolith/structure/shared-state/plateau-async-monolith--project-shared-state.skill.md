---
name: plateau-async-monolith--project-shared-state
description: Classical NgRx Store hosting global, cross-cutting state slices — auth-slice skeleton (still no real session lifecycle) plus the new connectivity slice for accurate online/offline detection — async-monolith plateau
domain: skill
type: template
...
/libs/shared/state
  /src
    /lib
      /auth
        [auth.actions.ts, auth.reducer.ts, auth.effects.ts, auth.selectors.ts](./classes/plateau-async-monolith--class-auth-store.skill.md)
      /connectivity
        [connectivity.actions.ts, connectivity.reducer.ts, connectivity.effects.ts, connectivity.selectors.ts](./classes/plateau-async-monolith--class-connectivity-store.skill.md)
      /notifications
      /offline-sync
    index.ts
```
