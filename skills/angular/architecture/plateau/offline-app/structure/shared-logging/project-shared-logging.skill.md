...
- `MIN_LOG_LEVEL` defaults to filtering out `debug`/`info` in production builds; `warn`/`error` are always enabled
- A log entry never contains an auth token, password, or other PII, regardless of level
- Only `warn`/`error`/`report()` entries ever reach the backend — `debug`/`info` stay local to the console sink
...
| ------------------ | ----------- | -------------- |
| logger.service.ts | Public API: `debug/info/warn/error/report(message, context?)` plus `forFeature(name)`. Forwards each entry to every registered `LogSink` after level filtering. | [[classes/class-logger-service.skill.md\|class-logger-service.skill]] |
...
| log-level.token.ts | `MIN_LOG_LEVEL` injection token, `'debug'` in dev / `'warn'` in prod | — |
| backend-log-sink.ts | Batches `warn`/`error`/`report()` entries and sends them to the backend, beacon-flushed on unload | [[classes/class-backend-log-sink.skill.md\|class-backend-log-sink.skill]] |
| log-retry-queue.ts | Bounded, IndexedDB-persisted retry queue for log batches that failed to send | [[classes/class-log-retry-queue.skill.md\|class-log-retry-queue.skill]] |
...
## MUST
- `LoggerService` MUST be the only class application code interacts with for logging; `ConsoleLogSink`/`BackendLogSink` MUST NOT be injected directly by feature code.
...
- **Wiring `BackendLogSink` to also forward `debug`/`info` entries "to have more data"**
  ...
  - Instead: keep `debug`/`info` local to `ConsoleLogSink`; use `report()` for anything worth sending deliberately
