---
name: class-permission-guard
description: Shared functional route guard factory checking a required permission against the auth slice's permission set — attached inside each feature's own routes
domain: skill
type: template
plateau: offline-app
artifact_type: guard
...
> The reusable `requirePermission` factory is defined once here, in `libs/shared/auth-ui`. Each feature imports it and attaches it inside its own `{feature}.routes.ts`, at the specific path it protects — consistent with hierarchical route ownership from `solution-app-routing`.
...
- Checks a permission string, never a role name, consistent with `class-has-permission-directive`
...
// permission.guard.ts (in libs/shared/auth-ui, imported by any feature)
