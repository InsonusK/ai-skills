...
> This plateau applies four of the five caching strategies defined by `solution-offline-first`. The fifth (stale-while-revalidate for federated remote chunks) is deferred — see the note at the end of this file.
...
`solution-offline-first`'s fifth caching rule — stale-while-revalidate for requests to a federated remote's origin (`KNOWN_REMOTE_ORIGINS`) — is **not** applied at this plateau. It depends on `RemoteRegistryService`, which is introduced by `solution-platform-embeddability`, not yet part of this plateau's solution set. There are no embeddable remotes to cache at this stage. This rule is re-added in full by the [[skills/angular/architecture/plateau/platform/plateau-platform.skill.md|platform]] plateau, where `KNOWN_REMOTE_ORIGINS` is sourced from `RemoteRegistryService`'s own runtime configuration.
