...
Runs every test project of the solution together — `@todo` scenarios excluded — and merges their Reqnroll scenarios, TRX counters, scenario results, and coverage (when `WITH_CODE_COVERAGE=true`) into one normalized result, per [[skills/common-workflow/test/solution-conformance-testing.skill/solution-conformance-testing.skill.md#report-contract|solution-conformance-testing]]. Writes `tmp/result/unit-test.json` and `tmp/result/scenarios.json` on a red run too, then exits with `dotnet test`'s own code. Verified with Reqnroll 3.3.4 + xUnit 2.9 on the VSTest runner, .NET 10 SDK.
...
```json
{
  "formatters": {
    ...
    "message": { "outputFilePath": "reqnroll_messages.ndjson" }
  }
}
```
