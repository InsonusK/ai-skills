...
Multi-property Value Objects persisted via EF Core `OwnsOne` must be configured in the entity's EF configuration (see [[skills/dotnet/architecture/artifacts/solutions/🧩validated/solution-domain-configuration.skill/solution-domain-configuration.skill|solution-domain-configuration]]):
...
# Check list
- [ ] All properties except GUID and Id (primary key and foreing key) which has invariant checks use [{ValueObject}](skills/dotnet/architecture/artifacts/solutions/🧩validated/solution-value-objects-and-rules.skill/Implementation/{Module}.Domain.csproj.extend/{ValueObject}.cs.create.md) which implements invariant checks
# Unittest TestCases
