...
- Stryker.NET mutation runs are slow on large modules; `make mutation-test` still supports `ONLY_DELTA`/`DELTA_BASE` for a fast local/manual run, even though no CI workflow currently calls it that way — the full, unscoped command is expected to run only on a post-merge report, never on a PR-gate.
