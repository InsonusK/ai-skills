...
- Logs with the stable `LogEvents.UnhandledException` event id from [[skills/dotnet/architecture/solutions/solution-app-logging.skill/solution-app-logging.skill|solution-app-logging]] — an unhandled exception is the single most alert-worthy line, so it carries a fixed id
...
`LogEvents` lives in `Shared.Logging` (from [[skills/dotnet/architecture/solutions/solution-app-logging.skill/solution-app-logging.skill|solution-app-logging]]); `BuildingBlocks` already references `Shared`.
