...
tags:
  ...
  - element/ientityversionresolver-cs
---
...
- Never throw when the entity is missing — return `0` so `ConcurrencyBehavior` can produce `Result.NotFound`
- Never reference EF Core or DbContext directly

## SHOULD
- Avoid returning a negative number for missing entities — complicates the contract
- Avoid implementing the resolver in App.Infrastructure or BuildingBlocks — Application owns per-entity data access
