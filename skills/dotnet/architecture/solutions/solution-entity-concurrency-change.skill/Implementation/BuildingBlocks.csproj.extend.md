...
- Never add EF Core dependency to BuildingBlocks
- Never define `IHasVersions`, `IEntityVersionResolver`, `IEntityVersionResolverFactory`, or `IVersioned` in BuildingBlocks — they belong in Shared
- Never add a generic `EntityByIdSpec<T>` to BuildingBlocks — per-entity specs belong in module Application projects

## SHOULD
- Avoid `ConcurrencyBehavior` constrained on `IRequest<T>` instead of `IHasVersions` — would check all commands including queries
- Avoid defining common concurrency contracts in BuildingBlocks — forces modules to reference BuildingBlocks for contracts
- Avoid duplicating `ByIdSpec` logic in BuildingBlocks instead of reusing module Application specs
