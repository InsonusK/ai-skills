...
- Keep the invariant condition local to the `{ValueObject}` (a `private static` predicate), so this solution stands alone; [[skills/dotnet/architecture/solutions/solution-domain-shared-rules.skill/solution-domain-shared-rules.skill|solution-domain-shared-rules]] (VP4) may later centralize it.
...
# Core Principle
- **Requires a domain layer** - `{ValueObject}` lives in `{Module}.Domain/ValueObjects` — this solution is VP3 and `depends_on solution-domain-behaviour` (VP1). Per the [Variability Map](skills/dotnet/architecture/variability-map.md), VP3 requires VP1 (directly, or transitively via Persistence).
...
SOLUTION:
- [[skills/dotnet/architecture/solutions/solution-domain-behaviour.skill/solution-domain-behaviour.skill|solution-domain-behaviour]]
  - [[skills/dotnet/architecture/solutions/solution-domain-behaviour.skill/Implementation/{Module}.Domain.csproj.create|{Module}.Domain.csproj]] - the project `{ValueObject}` is created in
  - [[skills/dotnet/architecture/solutions/solution-domain-behaviour.skill/Implementation/Shared.csproj.extend/DomainException.cs.create|DomainException.cs]] - thrown on an invalid value
- [[skills/dotnet/architecture/solutions/solution-soft-value-objects.skill/solution-soft-value-objects.skill|solution-soft-value-objects]]
  - [[skills/dotnet/architecture/solutions/solution-soft-value-objects.skill/Implementation/{Module}.Interfaces.csproj.extend/Soft{ValueObject}.cs.create|Soft{ValueObject}.cs]] - the base type `{ValueObject}` inherits
...
PROJECT:
- [[skills/dotnet/architecture/solutions/solution-value-objects.skill/Implementation/{Module}.Domain.csproj.extend|{Module}.Domain.csproj]] - extend - add a `/ValueObjects` folder, reference `{Module}.Interfaces`
  - [[skills/dotnet/architecture/solutions/solution-value-objects.skill/Implementation/{Module}.Domain.csproj.extend/{ValueObject}.cs.create|{ValueObject}.cs]] - create - strict type, inherits `Soft{ValueObject}`, validates in constructor
  - [[skills/dotnet/architecture/solutions/solution-value-objects.skill/Implementation/{Module}.Domain.csproj.extend/{Entity}.cs.extend|{Entity}.cs]] - extend - use `{ValueObject}` on entity properties
...
## MUST
- [[skills/dotnet/architecture/solutions/solution-value-objects.skill/Implementation/{Module}.Domain.csproj.extend#MUST|{Module}.Domain.csproj]]
  - [[skills/dotnet/architecture/solutions/solution-value-objects.skill/Implementation/{Module}.Domain.csproj.extend/{ValueObject}.cs.create#MUST|{ValueObject}.cs]]
