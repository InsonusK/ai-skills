...
# Check list
- [ ] All properties except `Id`/`Version`/GUID foreign keys that have invariant checks use a `{ValueObject}` per [[skills/dotnet/architecture/solutions/solution-value-objects.skill/Implementation/{Module}.Domain.csproj.extend/{ValueObject}.cs.create|{ValueObject}.cs]]
