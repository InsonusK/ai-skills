...
**AS IS** (from `solution-infrastructure-project` + `solution-repository-integration`, both hard dependencies):
```
/App.Infrastructure
  /Persistence
    AppDbContext.cs
...
- Avoid `UnitOfWork` exposing `BeginTransaction` — violates single-responsibility contract
