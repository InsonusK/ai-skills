...
# Goals
- Add the REST transport to the shared `ApiRegistration` partial (from [[skills/dotnet/architecture/solutions/solution-api-project.skill/solution-api-project.skill|solution-api-project]]) — `AddHttpApi()` registers controllers, ProblemDetails, and one Swagger doc per module; `UseHttpApi()` maps them.
