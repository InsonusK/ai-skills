...
PROJECT:
- [[skills/dotnet/architecture/solutions/🧩validated/solution-entity-classification.skill/Implementation/{Module}.Domain.csproj.extend|{Module}.Domain.csproj]] - extend - apply the correct entity class and configuration variant for the selected classification
  - [[skills/dotnet/architecture/solutions/🧩validated/solution-entity-classification.skill/Implementation/{Module}.Domain.csproj.extend/{EntityName}.cs.extend|{EntityName}.cs]] - extend - add `Guid`, `Version`, `IVersioned`, or none based on classification
  - [[skills/dotnet/architecture/solutions/🧩validated/solution-entity-classification.skill/Implementation/{Module}.Domain.csproj.extend/{EntityName}Config.cs.extend|{EntityName}Config.cs]] - extend - add unique `Guid` index, `xmin` concurrency token, or none based on classification
