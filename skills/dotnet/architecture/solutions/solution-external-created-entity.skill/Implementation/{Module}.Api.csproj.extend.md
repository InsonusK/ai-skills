...
- Never put business logic or domain rules in the extension method
- Never return ProblemDetails for `ConflictResult<Create{Entity}Result>` — the idempotent create contract returns the existing result

## SHOULD
- Avoid mapping `ConflictResult<T>` to a ProblemDetails body — breaks the idempotent create contract
- Avoid inline conflict mapping in every controller action instead of a shared extension
