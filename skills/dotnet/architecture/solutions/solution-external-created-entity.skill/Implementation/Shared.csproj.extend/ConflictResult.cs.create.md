...
- Never carry additional metadata beyond the conflict value
- Never throw exceptions in the constructor
- Never be defined in BuildingBlocks — it is a primitive used by both Application and BuildingBlocks layers

## SHOULD
- Avoid returning `Result<T>.Conflict()` and losing the existing entity result
- Avoid storing the conflict payload in `Errors` or `Location` instead of `Value`
- Avoid defining `ConflictResult<T>` in BuildingBlocks — forces Shared to reference BuildingBlocks
