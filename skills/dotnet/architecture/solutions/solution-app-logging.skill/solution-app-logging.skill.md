...
adr:
  - "[[skills/dotnet/architecture/solutions/solution-app-logging.skill/adr/console-first-extensible-sink|Console provider first, sink-extensible, no read-back]]"
---
...
SOLUTION:
- [[skills/dotnet/architecture/solutions/solution-sln-structure.skill/solution-sln-structure.skill|solution-sln-structure]]
  - [[skills/dotnet/architecture/solutions/solution-sln-structure.skill/Implementation/App.Host.csproj.create|App.Host.csproj]] - hosts `LoggingRegistration` and the `AddAppLogging()` call in `Program.cs`
  - [[skills/dotnet/architecture/solutions/solution-sln-structure.skill/Implementation/Shared.csproj.create|Shared.csproj]] - hosts the `LogEvents` catalogue
...
- (versions declared in `Directory.Packages.props` per [[skills/dotnet/architecture/solutions/solution-central-package-management.skill/solution-central-package-management.skill|solution-central-package-management]])

# Template Skill Mutations

PROJECT:
- [[skills/dotnet/architecture/solutions/solution-app-logging.skill/Implementation/App.Host.csproj.extend|App.Host.csproj]] - extend - configure the logging provider and levels in the composition root
  - [[skills/dotnet/architecture/solutions/solution-app-logging.skill/Implementation/App.Host.csproj.extend/LoggingRegistration.cs.create|LoggingRegistration.cs]] - create - `AddAppLogging()` — provider + level configuration, the one place a sink is swapped
- [[skills/dotnet/architecture/solutions/solution-app-logging.skill/Implementation/Shared.csproj.extend|Shared.csproj]] - extend - host the shared `EventId` catalogue
  - [[skills/dotnet/architecture/solutions/solution-app-logging.skill/Implementation/Shared.csproj.extend/LogEvents.cs.create|LogEvents.cs]] - create - stable `EventId` constants for the searched-for log lines
...
## MUST
- [[skills/dotnet/architecture/solutions/solution-app-logging.skill/Implementation/App.Host.csproj.extend/LoggingRegistration.cs.create#MUST|LoggingRegistration.cs]]
- [[skills/dotnet/architecture/solutions/solution-app-logging.skill/Implementation/Shared.csproj.extend/LogEvents.cs.create#MUST|LogEvents.cs]]
