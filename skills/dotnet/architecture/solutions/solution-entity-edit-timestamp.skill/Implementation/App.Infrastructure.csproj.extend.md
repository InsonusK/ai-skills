...
- Never set user timestamps.
- Never assign server timestamps anywhere outside `AppDbContext`.

## SHOULD
- Avoid overriding only `SaveChangesAsync` and forgetting the synchronous path.
- Avoid using `DateTime.Now` or `DateTime.UtcNow` instead of `DateTimeOffset.UtcNow`.
