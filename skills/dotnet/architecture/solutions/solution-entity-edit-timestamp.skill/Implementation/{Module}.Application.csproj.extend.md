...
- Never set server timestamps in the handler.
- Never set user timestamps in `AppDbContext`.

## SHOULD
- Avoid duplicating timestamp validation in every handler.
- Avoid assigning user timestamps without casting to the mutable interface — fails when entity setters are `internal`.
- Avoid forgetting to set `UserUpdatedDateTime` on creation for mutable entities.
