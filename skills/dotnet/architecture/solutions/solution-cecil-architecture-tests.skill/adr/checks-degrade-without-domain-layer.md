...
[[skills/dotnet/architecture/solutions/solution-domain-shared-rules.skill/adr/rules-project-references-interfaces-only|VP4 is not gated on VP1]] — a rules-only module (validation/policy, no entities) can apply it. For such a module the bottom two checks have nothing to scan.
