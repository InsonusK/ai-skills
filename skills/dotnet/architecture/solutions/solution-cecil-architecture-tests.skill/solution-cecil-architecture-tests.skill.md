...
- Read compiled IL via Mono.Cecil, don't execute it — a check over `MethodDefinition.Body.Instructions` is a fact about what the shipped code does, not a black-box assertion about what it returns for one input. See [[skills/dotnet/architecture/solutions/solution-cecil-architecture-tests.skill/adr/cecil-over-reflection|the ADR]] for why Cecil, not Reflection or a Roslyn analyzer.
...
- Each of the four checks is paired with a `.feature` file that describes, in plain language, what it proves — but the file is documentary, not executable Gherkin: the actual proof is the `[Fact]` itself, since a structural fact ("is this dead," "is this thrown from the right place") has no natural Given/input → Then/output shape a Reqnroll step could meaningfully parameterize. Scenario titles mirror the `[Fact]` method name 1:1 so a reader can find the corresponding test without a real step-definition binding to follow. See [[skills/dotnet/architecture/solutions/solution-cecil-architecture-tests.skill/Implementation/{Module}.Domain.Tests.csproj.extend/{Check}.feature.create|{Check}.feature]]

# Boundaries
- This documentary-`.feature` allowance is scoped to structural/architecture facts specifically — it is not a general license to skip step-definition bindings elsewhere in the codebase; every other `.feature` file in this catalog (including `{Module}.Domain.Rules.Spec`'s own rule-correctness scenarios) still needs a real, executing binding per [[skills/dotnet/architecture/solutions/solution-dotnet-conformance-testing.skill/solution-dotnet-conformance-testing.skill|solution-dotnet-conformance-testing]]'s own MUST rule
...
- `GuardedPropertyRuleCoverageTests` only scans `{Module}.Domain` — it does not scan `{Module}.Domain.Rules`, `{Module}.Application`, or any other module's assembly; narrowing a guarded setter to `private` is what actually closes an external bypass, this test only catches what's left reachable within `Domain` itself (see [[skills/dotnet/architecture/solutions/solution-cecil-architecture-tests.skill/examples/guarded-property-coverage|guarded-property-coverage.md]])
- This solution proves the rule *mechanism* is wired correctly as a whole — it does not prove any individual rule's own condition is correct; that is `{Module}.Domain.Rules.Tests`'s job, via [[skills/dotnet/architecture/solutions/solution-domain-shared-rules.skill/solution-domain-shared-rules.skill|solution-domain-shared-rules]]
...
PROJECT:
- [[skills/dotnet/architecture/solutions/solution-cecil-architecture-tests.skill/Implementation/{Module}.Domain.Rules.Tests.csproj.extend|{Module}.Domain.Rules.Tests.csproj]] - extend - Add the `Architecture/` folder for the two checks that scan `{Module}.Domain.Rules` only (always run with VP4)
  - [[skills/dotnet/architecture/solutions/solution-cecil-architecture-tests.skill/Implementation/{Module}.Domain.Rules.Tests.csproj.extend/{Module}RuleArchitectureTests.cs.create|{Module}RuleArchitectureTests.cs]] - create - dead-rule detection + code-uniqueness/format, one class, two `[Fact]`s — see [[skills/dotnet/architecture/solutions/solution-cecil-architecture-tests.skill/examples/dead-rule-detection|dead-rule-detection.md]], [[skills/dotnet/architecture/solutions/solution-cecil-architecture-tests.skill/examples/code-uniqueness-format|code-uniqueness-format.md]]
- [[skills/dotnet/architecture/solutions/solution-cecil-architecture-tests.skill/Implementation/{Module}.Domain.Tests.csproj.extend|{Module}.Domain.Tests.csproj]] - extend - Add the `Architecture/` folder for the two checks that need `{Module}.Domain` entities + `DomainException` (VP1-gated)
  - [[skills/dotnet/architecture/solutions/solution-cecil-architecture-tests.skill/Implementation/{Module}.Domain.Tests.csproj.extend/{Module}ArchitectureTests.cs.create|{Module}ArchitectureTests.cs]] - create - exception-type scoping, one class, one `[Fact]` — see [[skills/dotnet/architecture/solutions/solution-cecil-architecture-tests.skill/examples/exception-scoping|exception-scoping.md]]
  - [[skills/dotnet/architecture/solutions/solution-cecil-architecture-tests.skill/Implementation/{Module}.Domain.Tests.csproj.extend/GuardedPropertyRuleCoverageTests.cs.create|GuardedPropertyRuleCoverageTests.cs]] - create - The registry-driven call-graph check, its own class — see [[skills/dotnet/architecture/solutions/solution-cecil-architecture-tests.skill/examples/guarded-property-coverage|guarded-property-coverage.md]]
  - [[skills/dotnet/architecture/solutions/solution-cecil-architecture-tests.skill/Implementation/{Module}.Domain.Tests.csproj.extend/{Check}.feature.create|{Check}.feature]] - create - Documentary `.feature` file per check (in whichever `Architecture/` folder its `[Fact]` lives), scenario titles mirroring `[Fact]` method names
...
- Keep a coverage registry (e.g. `RequiredRuleChecks`) inside the test project, never in `Domain`/`Domain.Rules` production code.
  ...
  - Risk: `Domain.Rules` referencing `Domain`'s Entity types (or vice versa, depending on which side holds the registry) creates a dependency a portable, other-service-consumable Rules project must not carry — see [[skills/dotnet/architecture/solutions/solution-cecil-architecture-tests.skill/adr/registry-driven-coverage-over-per-rule-tests]].
...
- Never rely on a bespoke, per-rule Cecil test as the only guard for "does every Entity method call this rule."
  ...
  - Fix: one generic, registry-driven test (see [[skills/dotnet/architecture/solutions/solution-cecil-architecture-tests.skill/examples/guarded-property-coverage]]); a new rule adds a dictionary entry, not a new test class.
...
- Give each check a documentary `.feature` file with no real step-definition binding, not a fabricated one that pretends to bind Given/When/Then to production code.
  ...
  - Fix: the `.feature` file documents intent only; the `[Fact]` next to it is the actual, sole proof. Say so explicitly in the file (see [[skills/dotnet/architecture/solutions/solution-cecil-architecture-tests.skill/Implementation/{Module}.Domain.Tests.csproj.extend/{Check}.feature.create|{Check}.feature]]).
