...
- `EveryDomainRuleCheck_IsCalledByProductionCodeOutsideRules` — collect every `Check()` / `IRuleBuilder`-extension method in `{Module}.Domain.Rules`; collect every `Code.Call`/`Code.Callvirt` target across every method body in the production assemblies; fail if any entry point is never called. Full worked implementation and rationale: [[skills/dotnet/architecture/solutions/solution-cecil-architecture-tests.skill/examples/dead-rule-detection|dead-rule-detection.md]].
- `RejectionCodes_AreUniqueAndFollowModuleDotClassDotReasonFormat` — collect every `f.HasConstant` `string` field named `*Code` in `{Module}.Domain.Rules`; assert each matches the `{ModuleName}\.[A-Za-z0-9]+\.[A-Za-z0-9]+` regex and the set has no duplicates. Full worked implementation: [[skills/dotnet/architecture/solutions/solution-cecil-architecture-tests.skill/examples/code-uniqueness-format|code-uniqueness-format.md]].
