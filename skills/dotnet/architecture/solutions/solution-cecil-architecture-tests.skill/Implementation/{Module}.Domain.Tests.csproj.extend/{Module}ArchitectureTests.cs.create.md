...
- `DomainException_IsThrownOnlyFromValueObjectsOrEntities` — for each exception type and its allowed namespace set, walk every method body outside the allowed namespaces for a `Code.Newobj` whose `DeclaringType.FullName` matches the exception type; fail on any match. Full worked implementation and rationale: [[skills/dotnet/architecture/solutions/solution-cecil-architecture-tests.skill/examples/exception-scoping|exception-scoping.md]].
