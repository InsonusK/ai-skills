...
A rule is always: bundle the values it needs into a wrapper (an existing `SoftVO` property, a `SoftVO` assembled ad hoc, or an anonymous tuple), then apply `IsValid()`/`IRuleBuilder`-extension/`Check()` to that wrapper — identical to [format-validation.md](../format-validation.md) in every case. What differs is only where the wrapper's values come from:
...
| Wrapper shape | Usually a named `SoftVO` | Named `SoftVO` (reusable concept) or anonymous tuple (rule-local) — see [semantic-validation.md](../semantic-validation.md) | Same choice as Semantic — see [domain-validation.md](../domain-validation.md) |

![wrapper-mechanism](./diagrams/wrapper-mechanism.mmd)

Three arrows into `W` at the top, one shared pipeline below it. Format's arrow is the container handing over a value it already has; Semantic's arrow assembles the wrapper from the container's own other fields, no I/O; Domain's arrow does the same assembly, but only after a `Load` step brings in values from somewhere else. Everything below `W` — `IsValid()`, the `IRuleBuilder` extension, `Check()`, and the two consumers (fail-fast VO/Entity, collect-all DtoValidator) — is identical regardless of which arrow was taken. Domain validation is, mechanically, Semantic validation preceded by a loading step: "Domain validation = data preload + semantic validation." The rule itself never knows or cares which arrow it came from.

The choice between a **named `SoftVO`** and an **anonymous tuple** for the wrapper is orthogonal to Format/Semantic/Domain — it is a separate decision per rule: name the wrapper when the combination of fields is itself a reusable domain concept (`Schedule` — a task's time window, potentially a real Entity property on its own), leave it an anonymous tuple when the combination has no meaning beyond this one comparison (`(Balance, Amount)`, `(ParentId, ChildId)`). A named wrapper additionally justifies a `PropertyValidator`-equivalent DI/isolated-testing layer (see [format-validation.md](../format-validation.md)); a rule-local tuple usually does not need one.
...
- Domain validation stops looking like a special case requiring its own testing/wiring story — it reuses every adapter and mutation-testing config already built for Semantic validation, plus one loading step that lives outside `Domain.Rules` entirely.
...
- Two wrapper shapes (named `SoftVO` vs. anonymous tuple) mean two syntactically different `IRuleBuilder` extension signatures (`IRuleBuilder<T, SoftVO>` vs. `IRuleBuilder<T, (A,B)>`/generic-over-`T:IEquatable<T>` variants) to recognize, even though they play the same role.

## Three separate mechanisms (Format ≠ Semantic ≠ Domain)

### Description

Keep Format, Semantic, and Domain validation as three independently-designed shapes, each with its own example, its own adapter story, and its own conventions for how the rule is declared and wired — the state of this solution before the unification above was made explicit.
...
- Three things to keep consistent instead of one; a fix or improvement discovered while working on Format validation (e.g. the `WithMessage` parameterization, the `Severity`-aware throw pattern) has to be manually ported to Semantic and Domain examples separately, and did in fact fall out of sync during this solution's own development.
