...
- Never modify entity state
- Never call `SaveChangesAsync`
- Never dispatch commands
- Never cross-module handler live in `{Module}.Application` — Application has no multi-module DB access
- Never cross-module handlers do not use `Include()` — all mapping is done in handler via `Select()` or manual projection
- Never single-module handler use DbContext directly — use `IReadRepository<T>`
# Unittest TestCases
...
## SHOULD
- Avoid using `IReadRepository<T>` for cross-module JOIN — repository is single-entity, use DbContext
- Avoid forgetting `AsNoTracking()` — causes unnecessary change tracking overhead
- Avoid using `Include()` followed by client-side mapping — projection belongs in `Select()`
- Avoid loading full entities and mapping afterwards — always project in LINQ or handler
