...
**AS IS**: `App.Queries` does not exist yet — nothing composed before this solution creates it. This solution is effectively the project's `create`, labeled `extend` only because it is described alongside the other project-level Implementation files in this catalog.
...
- Never single-module queries live here — belongs in `{Module}.Application`
- Never app.Queries handlers modify entity state
- Never app.Queries handlers call `SaveChangesAsync`

## SHOULD
- Avoid single-module query handler placed in App.Queries — adds unnecessary cross-module machinery
- Avoid cross-module handler placed in `{Module}.Application` — Application has no access to other module's entity types
