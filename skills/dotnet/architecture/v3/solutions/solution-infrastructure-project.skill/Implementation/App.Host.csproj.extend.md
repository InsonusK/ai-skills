...
**AS IS** (from `plateau-stateless-non-interactive-service`, via `solution-sln-structure` + `solution-pipeline-registration`):
```
/App.Host
  /DependencyInjection
...
## MUST NOT
- Concern-specific registration code (a `DbContext`, a cache client, ...) added directly inside `AddInfrastructure()` by this solution — that belongs to whichever solution extends this method for its own concern
- `AddInfrastructure()` called more than once, or from anywhere other than `Program.cs`
