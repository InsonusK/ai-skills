...
## MUST NOT
- Reference `{Module}.Application`/`{Module}.Domain` from `App.Host` on account of this solution — it still only touches `{Module}.Api` and `{Module}.Interfaces`, per `solution-sln-structure`
