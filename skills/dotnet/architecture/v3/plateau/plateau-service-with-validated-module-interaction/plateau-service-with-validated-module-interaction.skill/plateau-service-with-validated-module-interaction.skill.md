...
parent_plateaus:
  - "[[skills/dotnet/architecture/v3/plateau/plateau-stateless-non-interactive-service/plateau-stateless-non-interactive-service.skill/plateau-stateless-non-interactive-service.skill.md|plateau-stateless-non-interactive-service]]"
standalone: false
adr:
  - "[[skills/dotnet/architecture/v3/plateau/plateau-service-with-validated-module-interaction/plateau-service-with-validated-module-interaction.skill/adr/restore-sample-api-and-greet-command.md|Restore Sample.Api and GreetCommand]]"
created_by:
...
- Cross-module validators: `{ValueObject}PropertyValidator`/`{Dto}Validator` are the *only* way another module checks a `Soft{ValueObject}`/DTO it received — resolved through `IValidator<T>` via DI, never by referencing this module's `Application`/`Domain` types. A condition needing preloaded data is a `{Feature}Check`, the only place a repository call would be allowed in this validation layer — this plateau defines its shape only, with `Load` deliberately throwing (this plateau has no repository yet, see next bullet); a persistence-introducing plateau supplies the concrete `Load` via its own `.extend.md` on this same class. See [[../../../solutions/solution-dto-property-validators.skill/solution-dto-property-validators.skill.md|solution-dto-property-validators]].
...
- validation
  ...
  - A cross-aggregate condition needing preloaded data can run before the handler too, via a `{Feature}Check` wired through `CustomAsync` — usable only once a persistence-introducing plateau gives the module something to load from (see Core Principles).
- commands
