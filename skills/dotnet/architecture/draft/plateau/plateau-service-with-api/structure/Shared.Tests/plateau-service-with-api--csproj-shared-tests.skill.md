...
# Core Principles
- Value-shaped scenarios: given one or more primitive values, prove how they compare/combine — never "is this input valid" (see [[./classes/plateau-service-with-api--class-shared-rule-steps.skill.md|class-shared-rule-steps]]). `Shared` is mostly interfaces and primitives today, so its own test suite is small.
...
- /Shared.Tests
  - /Rules
    - {Rule}.feature
  - /StepDefinitions
    - [{Rule}Steps.cs](./classes/plateau-service-with-api--class-shared-rule-steps.skill.md)
  - Shared.Tests.csproj
...
| /StepDefinitions | Bindings that call `Shared`'s real primitives | [[./classes/plateau-service-with-api--class-shared-rule-steps.skill.md\|class-shared-rule-steps]] |
