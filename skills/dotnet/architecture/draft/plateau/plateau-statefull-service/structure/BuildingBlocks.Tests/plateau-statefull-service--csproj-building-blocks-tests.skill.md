...
# Core Principles
- Technical-contract-shaped scenarios: proving a pipeline behavior's observable contract (e.g. `ExceptionHandlingBehavior`'s catch-log-return-generic-error contract) as its own `.feature` file, not just plain xUnit (see [[./classes/plateau-statefull-service--class-rule-steps.skill.md|class-rule-steps]]).
...
| /StepDefinitions | Bindings that exercise `BuildingBlocks`'s real behaviors | [[./classes/plateau-statefull-service--class-rule-steps.skill.md\|class-rule-steps]] |
