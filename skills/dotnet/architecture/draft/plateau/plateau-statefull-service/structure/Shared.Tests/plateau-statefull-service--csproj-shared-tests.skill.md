...
# Core Principles
- Value-shaped scenarios: given one or more primitive values, prove how they compare/combine — never "is this input valid" (see [[./classes/plateau-statefull-service--class-rule-steps.skill.md|class-rule-steps]]). `Shared` is mostly interfaces and primitives today, so its own test suite is small.
...
| /StepDefinitions | Bindings that call `Shared`'s real primitives | [[./classes/plateau-statefull-service--class-rule-steps.skill.md\|class-rule-steps]] |
