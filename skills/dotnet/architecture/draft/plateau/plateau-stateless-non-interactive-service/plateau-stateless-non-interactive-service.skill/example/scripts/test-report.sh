#!/usr/bin/env bash
# Assembles public/ from tmp/result/*.json summaries and tmp/report/* native reports.
set -euo pipefail
...
for required in "$RESULT_DIR/unit-test.json" "$RESULT_DIR/mutation-test.json" "$REPORT_DIR/tests" "$REPORT_DIR/mutation"; do
  if [ ! -e "$required" ]; then
    echo "test-report: expected $required to exist - did unit-test/mutation-test run first?" >&2
    exit 1
  fi
done
