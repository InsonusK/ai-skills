...
- [[skills/dotnet/architecture/draft/solutions/solution-conformance-testing.skill/solution-conformance-testing.skill.md|solution-conformance-testing]]
  - [[skills/dotnet/architecture/draft/solutions/solution-conformance-testing.skill/Implementation/{Module}.Domain.Tests.csproj.create.md|{Module}.Domain.Tests.csproj]] — hosts the `Architecture/` folder these tests live in, already wired into the module's `dotnet test`/`make unit-test` run
...
- Give each check a documentary `.feature` file with no real step-definition binding, not a fabricated one that pretends to bind Given/When/Then to production code.
  - Violation: writing hollow `[Given]`/`[When]`/`[Then]` step methods that exist only to satisfy `solution-conformance-testing`'s "every scenario has a binding" rule, without the step actually calling anything meaningful.
