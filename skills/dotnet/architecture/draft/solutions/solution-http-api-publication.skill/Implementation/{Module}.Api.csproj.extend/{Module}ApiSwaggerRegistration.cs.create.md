...
Consumed from `App.Host`'s `ApiRegistration` — see [[skills/dotnet/architecture/draft/solutions/solution-http-api-publication.skill/Implementation/App.Host.csproj.extend/ApiRegistration.cs.create.md|ApiRegistration.cs]] for the `AddSwaggerGen`/`UseSwaggerUI` wiring that reads `DocumentName`/`Title`/`Version`/`MatchesRoute` from every module's registration class.
