...
The rule (`AccountWithdrawalRule`) does not change between the same-aggregate and Try/Confirm cases — only how and when it gets called changes. See [[skills/dotnet/architecture/draft/solutions/solution-domain-rules.skill/solution-domain-rules.skill#boundaries|Boundaries]] for why the saga's Handler/Consumer wiring is not part of this solution.
