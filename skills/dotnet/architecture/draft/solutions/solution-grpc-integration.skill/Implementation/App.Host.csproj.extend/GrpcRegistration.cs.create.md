...
## MUST NOT
- Call `AddGrpc()`/`MapGrpcService<T>()` directly from `Program.cs`
- Assume `ApiRegistration`'s `AddApi()`/`UseApi()` were called — this file must work whether or not `solution-http-api-publication` is applied
