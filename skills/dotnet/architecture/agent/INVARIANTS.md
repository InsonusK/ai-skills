...
- **Carve-out:** `solution-dotnet-conformance-testing` legitimately `depends_on` skills outside those trees — `skills/common-workflow/test/solution-conformance-testing.skill` (the stack-agnostic parent it implements) and `skills/devops/workflows/devops-github-wf-pull-request.skill`/`skills/devops/workflows/devops-github-wf-release-test-report.skill` (the CI workflows that consume its `make` targets). These are the only allowed external `depends_on` in the catalog.
