...
## MUST NOT
- Redirect a condition that is not actually duplicated elsewhere — a condition used by exactly one consumer stays local, per `solution-value-objects`/`solution-domain-behaviour`'s own Boundaries
