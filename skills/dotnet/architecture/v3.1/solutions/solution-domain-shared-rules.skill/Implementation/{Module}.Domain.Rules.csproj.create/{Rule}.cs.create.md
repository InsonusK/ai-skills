...
The rule (`AccountWithdrawalRule`) does not change between the same-aggregate and Try/Confirm cases — only how and when it gets called changes. See [[skills/dotnet/architecture/v3.1/solutions/solution-domain-shared-rules.skill/solution-domain-shared-rules.skill.md#boundaries|Boundaries]] for why the saga's Handler/Consumer wiring is not part of this solution.
