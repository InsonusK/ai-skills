...
- Give each check a documentary `.feature` file with no real step-definition binding, not a fabricated one that pretends to bind Given/When/Then to production code.
  ...
  - Fix: the `.feature` file documents intent only; the `[Fact]` next to it is the actual, sole proof. Say so explicitly in the file (see [[skills/dotnet/architecture/v3.1/solutions/solution-cecil-architecture-tests.skill/Implementation/{Module}.Domain.Tests.csproj.extend/{Check}.feature.create.md|{Check}.feature]]).
