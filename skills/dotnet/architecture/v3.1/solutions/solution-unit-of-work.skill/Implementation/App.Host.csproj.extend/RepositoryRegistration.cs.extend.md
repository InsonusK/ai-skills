...
Extend `RepositoryRegistration` from [[skills/dotnet/architecture/v3.1/solutions/solution-repository-integration.skill/solution-repository-integration.skill.md|solution-repository-integration.skill]] with `IUnitOfWork` and `UnitOfWorkContext`:
...
- Never register as `Singleton` or `Transient`

## SHOULD
- Avoid `services.AddTransient<IUnitOfWork, UnitOfWork>()` — breaks DbContext scope sharing
- Avoid `services.AddSingleton<UnitOfWorkContext>()` — depth counter leaks across requests
