...
## SHOULD
- Avoid `UnitOfWorkBehavior` constrained on `IRequest<T>` instead of `ICommand<TResponse>` — would commit on queries
- Avoid `UnitOfWorkBehavior` constrained on bare `ICommand` — the non-generic marker no payload-bearing command implements, so the behavior would never activate on any real command
