...
- Never add FluentValidation, Ardalis.Result, or EF Core packages to Shared
- Never add implementation code to Shared
- Never `IQuery` extend `ICommand` — queries must remain distinct from write-side markers

## SHOULD
- Avoid defining `IQuery` in BuildingBlocks — forces modules to reference BuildingBlocks for contracts
- Avoid `IQuery` extending `ICommand` — would blur the boundary between read and write operations
