...
- Never expose domain entity types as properties
- Never have public setters — `record` provides immutability
- Never query validator duplicates rules already defined in `{ValueObject}PropertyValidator` or `{Dto}Validator` from `solution-dto-property-validators.skill`
...
- Avoid DTO containing a domain entity — always project to flat primitives
- Avoid DTO with mutable properties — use `record` for immutability
