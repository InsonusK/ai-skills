...
- Never contain methods or logic
- Never reference domain entity types as properties
- Never extend `ICommand` or `ICommand<TResponse>` — queries must remain distinct from write-side markers
- Never query handler inject `IRepository<T>` — signals write intent, use `IReadRepository<T>`
- Never query handler inject `IUnitOfWork` or call `SaveChangesAsync`
- Never query handler modify entity state or dispatch commands
