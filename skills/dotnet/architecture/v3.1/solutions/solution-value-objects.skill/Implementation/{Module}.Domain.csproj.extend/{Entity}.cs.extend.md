...
**AS IS** (entity from `solution-domain-behaviour` — property is a `Soft{ValueObject}` or primitive):
```csharp
public class Order
{
...
- Never use a primitive type on an Entity property when the value carries business meaning or invariant constraints
- Never expose a primitive Entity property when a Value Object could enforce the same invariants

# Check list
- [ ] All properties except `Id`/`Version`/GUID foreign keys that have invariant checks use a `{ValueObject}` per [[skills/dotnet/architecture/v3.1/solutions/solution-value-objects.skill/Implementation/{Module}.Domain.csproj.extend/{ValueObject}.cs.create.md|{ValueObject}.cs]]
