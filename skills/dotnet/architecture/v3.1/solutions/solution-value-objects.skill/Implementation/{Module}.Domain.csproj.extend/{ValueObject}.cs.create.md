...
- Never depend on repositories, `DbContext`, or any service
- Never depend on a separate rules project — the predicate is local to this file
- Never expose public setters
- Never redeclare a property that `Soft{ValueObject}` already declares
- Never be used to carry identity — use the entity `Id` for that
