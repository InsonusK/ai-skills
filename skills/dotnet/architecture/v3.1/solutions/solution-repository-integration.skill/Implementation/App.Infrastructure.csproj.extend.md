...
- Never call `SaveChangesAsync` inside `Repository<T>`
- Never create per-entity repository subclasses
- Never configure entities inline in `AppDbContext.OnModelCreating` beyond the single `ApplyConfigurationsFromAssembly` call — per-entity mapping belongs in that entity's own `{Entity}Config` (see `solution-domain-configuration`)

## SHOULD
- Avoid `TaskRepository : Repository<TodoTask>` — unnecessary subclass, generic handles all types
- Avoid manual `SpecificationEvaluator.Default.GetQuery(...)` calls — Ardalis base handles this
