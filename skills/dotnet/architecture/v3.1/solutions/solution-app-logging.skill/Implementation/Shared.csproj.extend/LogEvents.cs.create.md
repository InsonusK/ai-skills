...
Ranges: `1xxx` lifecycle, `2xxx` pipeline, `5xxx` failures. A new solution that needs a stable event adds a constant in the right range.
...
## SHOULD
- Keep the numeric ranges (`1xxx`/`2xxx`/`5xxx`) meaningful so an id hints at its category.
