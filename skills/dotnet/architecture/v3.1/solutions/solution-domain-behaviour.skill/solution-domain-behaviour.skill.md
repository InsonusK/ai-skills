...
# Capabilities
- A `{Module}.Domain` project referencing only `Shared` and `{Module}.Interfaces` — no infrastructure, no EF Core (that arrives with persistence).
...
# Boundaries
- EF Core, persistence configuration, and repositories are **not** added here — `{Module}.Domain` references only `Shared` and `{Module}.Interfaces`. `solution-domain-configuration` / `solution-repository-integration` (VP2) add the persistence concerns when selected.
...
- Keep `{Module}.Domain` referencing only `Shared` and `{Module}.Interfaces`.
  - Risk: an EF Core or infrastructure reference here makes every module with a domain layer carry persistence, contradicting VP1/VP2 independence.
  - Fix: persistence concerns are added by VP2 solutions, not this one.
...
# Check list
- [ ] `{Module}.Domain.csproj` created, referencing only `Shared` + `{Module}.Interfaces`.
...
- [ ] No EF Core / infrastructure reference in `{Module}.Domain`.
