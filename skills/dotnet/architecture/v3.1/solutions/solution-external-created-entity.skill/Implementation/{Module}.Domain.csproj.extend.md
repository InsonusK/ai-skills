...
- Never `Guid` used in domain logic, domain events, or as a foreign key in relationships
- Never `Guid` reassigned after entity creation
- Never internal entity types (no external creation) have `Guid`

## SHOULD
- Avoid `Guid` with `public set` — application code must never modify it
- Avoid `Guid` used in domain method logic — it is a correlation handle only
