...
- Never set user timestamps in `OnBeforeSaving()`.
- Never use `DateTime` or `DateTime.Now`.
...
- Never validate `ActionTimeStamp` inside handlers.
- Never set server timestamps in handlers.
- Never allow `ActionTimeStamp` to be in the future.
- Never use EF attributes on entities for timestamp mapping.

## SHOULD
- Avoid overriding only `SaveChangesAsync`.
- Avoid assigning server timestamps in handlers or behaviors.
- Avoid iterating over all entries instead of filtering by interface.
