...
- Never add behavior logic or dependencies to Shared timestamp contracts.
- Never define timestamp markers in BuildingBlocks or module projects.

## SHOULD
- Place the timestamp command marker in `Shared.Timestamps` to keep it independent of MediatR internals.
- Avoid putting timestamp contracts in `{Module}.Interfaces` — would force cross-module consumers to reference a specific module.
- Avoid adding validation logic to `ICommandWithTimestamp` — interfaces are contracts, not behaviors.
