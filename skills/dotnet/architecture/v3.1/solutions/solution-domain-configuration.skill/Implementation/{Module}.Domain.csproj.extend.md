...
- Never place EF config classes outside /Configurations folder
- Never use EF data annotations on domain entity classes
- Never put mapping logic directly in `DbContext.OnModelCreating`
- Never configure cross-module foreign keys here

## SHOULD
- Avoid annotating domain entity with `[Column]`, `[Index]`, `[ForeignKey]` — all mapping belongs in config class
- Avoid registering configs manually one by one in DbContext — use `ApplyConfigurationsFromAssembly`
- Avoid sharing one config class across multiple entity types
- Avoid placing cross-module FK config in Domain — belongs in App.Infrastructure
