...
- Async cross-aggregate rules that already have a sync `Check()` reuse it as-is via `CustomAsync` + `context.AddFailure(failure)` — no re-declaration of `ErrorCode`/`Message` at the async call site.
...
- Every adapter (VO, PropertyValidator, DtoValidator) needs its own translation code between `RuleResult` and whatever FluentValidation expects downstream — an extra layer that produced no observable benefit.
...
- Replaced by decentralized `public const string ...Code`/`...Message` declared next to the rule that produces them, format `{ModuleName}.{Class}.{Reason}`, with an architecture test asserting uniqueness across the `Domain.Rules` assembly instead of a file a human has to read.
