...
- Never bind an `@format`-tagged scenario — those have no DtoValidator/`{Feature}Check` adapter to prove
- Never mock or stub the rule's own `Check()`/`IsValid()` — only the data-loading dependency is a legitimate mock target here
