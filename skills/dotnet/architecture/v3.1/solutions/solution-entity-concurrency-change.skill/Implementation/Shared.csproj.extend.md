...
- Never add MediatR, EF Core, or JSON serialization dependencies to Shared for these contracts
- Never place implementations in Shared

## SHOULD
- Avoid defining `IHasVersions`, `IEntityVersionResolverFactory`, or `IEntityVersionResolver` in BuildingBlocks — forces modules to reference BuildingBlocks for contracts
