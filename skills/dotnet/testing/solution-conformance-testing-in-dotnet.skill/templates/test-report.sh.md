...
`report-template/index.html` is a small static page the project owns (links to `tests/`, `coverage/`, `mutation/`) — not generated by this script. It lives outside `.github/` since this solution never owns `.github/workflows/*` — publishing it is a separate CI concern this solution never owns.
