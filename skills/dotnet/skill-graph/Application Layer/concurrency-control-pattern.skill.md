...
- EF concurrency token configured on all mutable entities — see [[entity-concurrency-pattern.skill]] 
MUST NOT:
...
# Relations
- [[entity-concurrency-pattern.skill]] — Version field on entity, IsConcurrencyToken EF config
- [[skills/dotnet/skill-graph/Domain Layer/command-handler-pattern.skill]] — update commands implement IHasVersions
- [[repository-pattern.skill]] — ConcurrencyBehavior uses IReadRepository to load entities
- [[api-structure.skill]] — ETag on GET, If-Match on PUT/PATCH, 412 response
- [[backend-project-structure.skill]] — IHasVersions in BuildingBlocks, resolver in App.Infrastructure