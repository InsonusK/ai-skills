---
uid: 3b15c16d-781e-43ff-bc13-e1a2eaf2b059
...
---
# Goal
Define how to implement a query handler inside [[skills/dotnet/skill-graph/developing/Module/Application csproj/module-application.csproj.skill|{ModuleName}.Application]]. A query handler is strictly read-only — it fetches and projects data, never modifies state. This skill covers single-module queries only. Cross-module JOIN queries live in [[skills/dotnet/skill-graph/developing/App/Queries csproj/app-queries.csproj.skill|App.Queries]] — see app-queries.skill.
...
- Query and DTOs declared in [[skills/dotnet/skill-graph/developing/Module/Interfaces csproj/module-interface.csproj.skill|{ModuleName}.Interfaces]] — handler implemented here
- Simple projection via spec; complex mapping done in handler

# Structure
## Place in csproj
Defines in [[skills/dotnet/skill-graph/developing/Module/Application csproj/module-application.csproj.skill|module-application.csproj.skill]]
```
...
```example
- class name
	- rule: QueryName + Handler suffix
	- pattern: {QueryName}Handler
	- example: GetTasksHandler
- file name:
	- rule: QueryName + .Handler.cs
	- pattern: {CommandName}.Handler.cs
	- example: GetTasks.Handler.cs 
```

## Implementation
### Query and DTO Declaration
Queries and DTOs live in [[skills/dotnet/skill-graph/developing/Module/Interfaces csproj/module-interface.csproj.skill|{ModuleName}.Interfaces]] — not in Application.
...
# Relations
- [[files/ardalis-specification.skill|ardalis-specification.skill]] — specs used for filtering and projection
- [[files/repository.skill|repository.skill]] — IReadRepository contract used here
- [[skills/dotnet/skill-graph/developing/Module/Application csproj/classes/module-application-command-handler.class.skill|module-application-command-handler.class.skill]] — counterpart for write operations
- [[skills/dotnet/skill-graph/developing/App/Queries csproj/app-queries.csproj.skill|app-queries.csproj.skill]] — cross-module JOIN queries belong there, not here
- [[skills/dotnet/skill-graph/developing/App/Host csproj/class/app-host-result-mapper.class.skill|app-host-result-mapper.class.skill]] — maps Result<T> to HTTP response
