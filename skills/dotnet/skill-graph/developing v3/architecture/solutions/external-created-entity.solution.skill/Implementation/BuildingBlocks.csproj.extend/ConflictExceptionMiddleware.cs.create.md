...
# Unittest TestCases
- [ ] WHEN applied THEN Catch any ConflictException thrown during an HTTP request
- [ ] WHEN applied THEN Write a 409 Conflict response with the existing entity body extracted via ex.GetValue()
- [ ] WHEN applied THEN Centralize 409 handling so controllers do not need per-action try/catch blocks
- [ ] WHEN applied THEN Catches non-generic ConflictException base class — handles any ConflictException<T> without knowing T
- [ ] WHEN applied THEN Writes application/json response body from ex.GetValue()
- [ ] WHEN applied THEN Does not catch non-Conflict exceptions — those propagate to the global exception handler
- [ ] WHEN applied THEN Registered early in the HTTP pipeline, after routing and authentication but before endpoints
- [ ] WHEN verified THEN ConflictExceptionMiddleware defined in BuildingBlocks/Middleware/ConflictExceptionMiddleware.cs
- [ ] WHEN verified THEN Catches non-generic ConflictException
- [ ] WHEN verified THEN Writes 409 with body from ex.GetValue()
- [ ] WHEN verified THEN Registered in App.Host HTTP pipeline
- [ ] WHEN naming 'Conflict exception middleware' THEN pattern matches convention
