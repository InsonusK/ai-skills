...
| Layer                | Project                    | Responsibility                                              | Detail                       |
| -------------------- | -------------------------- | ----------------------------------------------------------- | ---------------------------- |
| Module — Api         | `{ModuleName}.Api`         | HTTP endpoints, MediatR dispatch                            | [[module-api.skill]]         |
| Module — Interfaces  | `{ModuleName}.Interfaces`  | Public contracts — commands, queries, DTOs, events          | [[module-layer.skill]]       |
| Module — Application | `{ModuleName}.Application` | Orchestration — handlers, validators, specs                 | [[module-application.skill]] |
| Module — Domain      | `{ModuleName}.Domain`      | Business logic — entities, VOs, rules, events               | [[module-domain.skill]]      |
| App.Host             | `App.Host`                 | Composition root — DI, pipeline, module wiring              | [[app-host.skill]]           |
| App.Infrastructure   | `App.Infrastructure`       | Persistence — DbContext, repos, outbox, messaging           | [[app-infrastructure.skill]] |
| App.Queries          | `App.Queries`              | Cross-module read models and JOIN queries                   | [[app-queries.skill]]        |
| Shared               | `Shared`                   | Cross-cutting primitives — Result, Exceptions               | [[shared-layer.skill]]       |
| BuildingBlocks       | `BuildingBlocks`           | Reusable framework patterns — pipeline behaviors, spec base | [[building-blocks.skill]]    |
...
# Relations
- [[module-layer.skill]] — module structure, 4 projects, inter-module rules
