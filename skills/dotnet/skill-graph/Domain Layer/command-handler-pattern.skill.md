...
See [[api-structure.skill]] for full HTTP mapping. Standard conventions:
...
- Sub-commands must be safe inside an existing unit of work — see [[repository-pattern.skill]] 
SHOULD:
...
# Relations
- [[repository-pattern.skill]] — IRepository, IUnitOfWork, UnitOfWorkBehavior
- [[ardalis-specification-pattern.skill]] — all entity loading uses specifications
- [[domain-event-pattern.skill]] — domain calls inside handler raise events collected by entity
- [[api-structure.skill]] — Result status mapped to HTTP responses in controller
- [[cross-module-interaction.skill]] — sub-commands are the cross-module write mechanism
- [[backend-project-structure.skill]] — handler and validator placement in Application/Features