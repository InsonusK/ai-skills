...
- The script lives at `scripts/validation_queue.py` inside this skill's folder; the repository root is always four directory levels up from the script, whether the skill runs deployed (`.agents/skills/skill-validation/`, `.claude/skills/skill-validation/`) or in-repo (`skills/common-workflow/skill-validation.skill/`).
