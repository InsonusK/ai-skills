...
- Name a bullet under `# Goal`, `# Core Principle`, or `# Rule` (`## MUST`/`## SHOULD`/`## MAY`) when it is long or dense enough to need one: prefix it as `**{Name}** - {description}`, where `{Name}` is a short English noun phrase (2-4 words) distinct from the description's own opening words — never a near-restatement of them. A bullet needs a name when its own text — excluding any nested `Violation`/`Risk`/`Fix` — exceeds ~20 words, or, for a `# Rule` bullet specifically, whenever it carries a nested `Violation`/`Risk`/`Fix` block at all, regardless of the bullet's own length. Leave a bullet unnamed when neither condition applies — most `# Goal` and `# Core Principle` bullets, and short `## MAY` bullets, stay unnamed. This rule governs skills being created or updated; it is not a mandate to retrofit bullets in a skill you are not otherwise touching.
  - Violation: naming a bullet `**Format** - Use links that are resolvable from the skill file` where the name only parrots the description's first word; or leaving a 30-word `## MUST` bullet, or one with nested `Risk`/`Fix`, without a name.
  - Risk: without a name, refreshing memory on a large or dense skill later requires re-reading full sentences — or whole `Violation`/`Risk`/`Fix` blocks — to recall what a bullet was even about; a name that just repeats the description's opening adds a line without giving any of that scanning benefit.
  - Fix: compress the bullet's actual point into a short, wording-distinct name, and apply it only at the threshold above — not to every bullet regardless of length, and not to `# Check list` items, which stay short verification checks.
...
- [ ] Any `# Goal`/`# Core Principle`/`# Rule` bullet over ~20 words, or any `# Rule` bullet with a nested `Violation`/`Risk`/`Fix`, starts with `**{Name}** - `; the name is a short English phrase distinct from the description's opening words, not a restatement of it.
