...
An **Application condition** is the boolean condition, stated over chosen [[skills/common-workflow/architecture/design/plateau-map/variability-map-create.skill/glossary/variant|Variants]], under which a [[skills/common-workflow/architecture/design/plateau-map/delta-conflict-detection.skill/glossary/delta-module|Delta module]] is applied during assembly.
...
Usually a conjunction over the specific VPs a delta actually depends on — never over every VP in the catalog. A [[skills/common-workflow/architecture/design/plateau-map/delta-conflict-detection.skill/glossary/conflict-resolving-delta|conflict-resolving delta]]'s Application condition is the conjunction of every VP its resolved solutions require (e.g. `S=Yes AND E=Yes`), keeping growth local to the pair (or group) that actually intersects.
