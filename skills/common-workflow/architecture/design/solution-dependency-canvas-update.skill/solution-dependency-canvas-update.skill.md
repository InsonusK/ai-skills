...
- A plateau is drawn as a canvas `group`-type node, labeled with the plateau's name. Every solution the plateau's `created_by` lists is positioned **inside** that group's bounds — membership is expressed by position, not by a drawn edge, exactly as in [[skills/common-workflow/architecture/example_architecture/example-architecture-overview.canvas|the worked example canvas]] and the user-authored sketch this convention is based on. Do not draw an edge for plain `created_by` membership.
