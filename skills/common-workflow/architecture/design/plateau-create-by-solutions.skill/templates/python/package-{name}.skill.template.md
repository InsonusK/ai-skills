...
- Fill `whenToUse` with the concrete package-level situations that require this skill (adding/editing a module inside `{App}`, deciding whether new code belongs here, checking allowed dependencies). See [skill-design](skills/design/skill-design.skill/skill-design.skill.md) for the baseline rules.
