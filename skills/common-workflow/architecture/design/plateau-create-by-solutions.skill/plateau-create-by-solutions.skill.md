...
- **`FDN` VPs never fork the tree** - Before naming a new plateau for a VP, check whether it classifies `FDN` in [[skills/common-workflow/architecture/design/plateau-map/delta-conflict-detection.skill/delta-conflict-detection.skill.md#fdn-never-forks-the-plateau-tree|delta-conflict-detection]] — a Plateau Component, or an alternative adapter behind an already-existing port (e.g. one outbox dispatch target vs another) — in which case it is a composition-root wiring choice, never a new plateau.
