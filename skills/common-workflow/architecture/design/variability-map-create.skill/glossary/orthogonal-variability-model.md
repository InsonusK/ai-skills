...
The **Orthogonal Variability Model** is a way of documenting variability as its own model — Variation Points, Variants, Constraints — kept separate ("orthogonal") from the model of the actual artifacts (code, solutions, plateaus), and connected to it only through explicit [[skills/common-workflow/architecture/design/variability-map-create.skill/glossary/realized-by.md|Realized by]] links. Introduced by Pohl, Böckle, and van der Linden (2005).

## Why it exists
A [[skills/common-workflow/architecture/design/variability-map-create.skill/glossary/feature-model.md|Feature Model]] draws the questions as a tree, but leaves open where the answers live in code — writing that connection directly into the tree makes it bloat with implementation detail as the catalog grows. OVM's fix is to never embed code references in the tree at all: keep a separate table (a Variability Map) and link outward.

## How it works
Three terms, always used together: a [[skills/common-workflow/architecture/design/variability-map-create.skill/glossary/variation-point.md|Variation Point]] (the question), a [[skills/common-workflow/architecture/design/variability-map-create.skill/glossary/variant.md|Variant]] (one legal answer), and [[skills/common-workflow/architecture/design/variability-map-create.skill/glossary/realized-by.md|Realized by]] (the link from that answer to the code realizing it). Adding a new axis of variability is one new table row, not a new tree node crossed with every existing node.
...
graph LR
    ...
    VP1 -. "requires" .-> VP2
```
The dotted edge is a Constraint between VPs, not a "build order" — the model records legality of combinations, not assembly sequence (assembly is [[skills/common-workflow/architecture/design/delta-conflict-detection.skill/glossary/delta-oriented-programming.md|Delta-Oriented Programming]]'s job).
