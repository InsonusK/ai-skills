---
name: plateau-integrated-service--file-config-config
description: internal/config/config.go of the plateau-integrated-service plateau
...
# Check list
- [ ] `LOG_LEVEL` defaults to `"info"`; `HTTP_LISTEN_PORT` defaults to `"8080"`; `GRPC_LISTEN_PORT` defaults to `"50051"`; `REPUTATION_ADDR` is required.
