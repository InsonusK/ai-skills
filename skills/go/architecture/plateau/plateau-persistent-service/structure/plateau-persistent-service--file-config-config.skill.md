---
name: plateau-persistent-service--file-config-config
description: internal/config/config.go of the plateau-persistent-service plateau
...
```
Verified against this plateau's own `example/internal/config/config.go` (`go build`/`go vet` clean; `DATABASE_DSN` unset correctly fails `Load` before any adapter is dialed).
