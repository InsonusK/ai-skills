...
func (h *stubLinkHistory) Record(ctx context.Context, entry interfaces.LinkHistoryEntry) error {
	if h.recordErr != nil {
		return h.recordErr
	}
...
func newWorld() *World {
	...
	history := &stubLinkHistory{}
	return &World{svc: services.NewLinkCheckService(rep, cache, history), reputation: rep, cache: cache, history: history}
}
