...
whenToUse: when a Go web-service needs to publish an event for other services to react to, and the publication is not itself triggered by a persisted-data change (otherwise see solution-transactional-outbox)
domain: skill
type: architecture
