...
# Core Principle
- This is a report-only workflow: it never blocks anything. The merge gate — `make unit-test` and the delta-scoped `make mutation-test` — lives in [[skills/devops/workflows/devops-github-wf-pull-request.skill/devops-github-wf-pull-request.skill.md|devops-github-wf-pull-request]]; this workflow only runs after a PR already merged.
...
- Fix: add `continue-on-error: true` to the mutation-test step itself, as shown in [example](./templates/release-test-report.example.md); the job still uploads its report and normalized score artifact regardless of the tool's exit code, and the PR-gate workflow already enforced the threshold pre-merge.
