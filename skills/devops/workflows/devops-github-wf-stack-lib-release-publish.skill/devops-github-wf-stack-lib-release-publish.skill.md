...
# Core Principle
- Gating mirrors [[skills/devops/workflows/devops-github-wf-docker-release-publish.skill/devops-github-wf-docker-release-publish.skill.md|devops-github-wf-docker-release-publish]] exactly: relevant-change detection via `./.github/actions/check-changes` (gate on `code`/`workflow`, not `docker`), never a `bumped` gate — [[skills/devops/workflows/devops-github-wf-pull-request.skill/devops-github-wf-pull-request.skill.md|devops-github-wf-pull-request]] already enforced the version bump before this code reached `master`.
- `unit-test` runs in parallel with `check-version` (both `needs: changes` only, never each other) — the same shape as `devops-github-wf-pull-request`'s `unit-test`/`version-check` pair and `devops-github-wf-docker-release-publish`'s own `unit-test` job. The final `publish` job additionally `needs: unit-test`, so a direct push to `master`/`develop` still can't publish a package built from code that fails its own tests.
...
Copy the `changes`, `check-version`, and `unit-test` jobs from [base-jobs.example.md](./templates/base-jobs.example.md) verbatim into every stack's workflow file — [[skills/devops/workflows/devops-github-wf-docker-release-publish.skill/devops-github-wf-docker-release-publish.skill.md|devops-github-wf-docker-release-publish]] and every `stack-lib-release-publish-in-{stack}` implementation share these three jobs byte-for-byte; only the final job (`docker-publish` vs. `publish`) differs.
...
- Risk: as soon as one workflow's `changes`/`check-version`/`unit-test` jobs differ from another's — even by a single renamed output or an extra combined boolean — the jobs stop being copy-paste identical, and every future change to change-detection, version-reading, or testing has to be re-applied by hand in multiple files instead of once.
- Fix: keep all three jobs exactly as [base-jobs.example.md](./templates/base-jobs.example.md) defines them, exposing the full, uniform output set; let each consumer's own final job read only the specific outputs it needs in its own `if:`.

### Never publish a package built from code that fails its own tests
Add `unit-test` to `publish`'s `needs`, and gate it with `if: always() && needs.changes.result == 'success' && needs.check-version.result == 'success' && needs.unit-test.result != 'failure' && (...)`, never a plain boolean `if:` with `unit-test` merely listed in `needs`.
...
- Risk: this exists specifically for a direct push to `master`/`develop` that bypassed [[skills/devops/workflows/devops-github-wf-pull-request.skill/devops-github-wf-pull-request.skill.md|devops-github-wf-pull-request]]'s PR gate — without it, a broken commit pushed straight to a protected branch still packs and publishes. But `unit-test` is itself conditionally skipped (no `code`/`test` change); GitHub's default `needs` gating treats a skipped job the same as a failed one, so a plain `if:` with no `always()` would wrongly cascade-skip `publish` on a push that changed only the workflow file.
- Fix: use the exact `always()`-based condition from [base-jobs.example.md](./templates/base-jobs.example.md); `!= 'failure'` (not `== 'success'`) is what lets a skipped `unit-test` still pass.
...
- Violation: publishing on every `master`/`develop` push regardless of whether anything relevant changed, or gating on `bumped` the way [[skills/devops/workflows/devops-github-wf-release-info-publish.skill/devops-github-wf-release-info-publish.skill.md|devops-github-wf-release-info-publish]] does.
...
- Fix: `publishable == 'true'` gates the workflow's existence; `code`/`workflow` changed gates each run, exactly as [[skills/devops/workflows/devops-github-wf-docker-release-publish.skill/devops-github-wf-docker-release-publish.skill.md|devops-github-wf-docker-release-publish]] does for the image.
...
# Example
See [base-jobs.example.md](./templates/base-jobs.example.md) for the shared `changes`/`check-version`/`unit-test` jobs every stack's implementation starts from. Each stack's own skill (`devops-github-wf-stack-lib-release-publish-in-{stack}`) shows only the `publish` job it adds on top.
...
- [ ] `changes`, `check-version`, and `unit-test` are copied from [base-jobs.example.md](./templates/base-jobs.example.md) unmodified — no pre-combined `relevant` output, no dropped `check-version` output.
- [ ] `publish` also `needs: unit-test` and uses the `always()`-based condition (`unit-test.result != 'failure'`) — never a plain `if:` that would cascade-skip on a skipped `unit-test`, and never `unit-test.result == 'success'`.
