...
Writes `tmp/result/scenarios.json` per [[skills/common-workflow/test/solution-conformance-testing.skill/solution-conformance-testing.skill.md#scenario-report|solution-conformance-testing's Scenario report]]. Stack-independent — the same script (unmodified) is used by the .NET and the TypeScript variants of this solution; it is duplicated verbatim in each rather than shared, so each solution stays self-contained. `scripts/unit-test.sh` calls it with the runner's per-scenario results already reduced to `[{"uri", "line", "status"}]`.
