...
Pure assembly — no `python`/test tooling involved, so this same script (unmodified) also works for the .NET and TypeScript variants of this solution; see [[skills/common-workflow/test/solution-conformance-testing.skill/solution-conformance-testing.skill.md#report-contract|solution-conformance-testing]]. It is duplicated verbatim in each stack solution rather than shared, so each solution stays self-contained and portable on its own.
